...
{
	internal class Game_Events : EventConsumer
	{
		EventManager EventManager;
		VoxelGameEnvironment GameEnv;
		byte[] Keyboard_Matrix;
		bool[] Mouse_Matrix;
		...
		~Game_Events()
		{
			Keyboard_Matrix = null;
			Mouse_Matrix = null;
		}

		internal virtual void SetEventManager( EventManager EventManager ) { this.EventManager = EventManager; }
		internal virtual void SetGameEnv( VoxelGameEnvironment GameEnv ) { this.GameEnv = GameEnv; }
		internal virtual void SetLastFrameTime( double LastFrameTime ) { this.LastFrameTime = LastFrameTime; }

		internal virtual void SetEnableMouseEvents() { EnableMouseEvents = true; }
		internal virtual void SetDisableMouseEvents() { EnableMouseEvents = false; }

		bool Is_KeyPressed( Key KeyCode, bool Reset )
		{
			bool IsPressed;

			IsPressed = ( Keyboard_Matrix[(int)KeyCode] != 0 ) ? true : false;
			if( Reset ) Keyboard_Matrix[(int)KeyCode] = 0;
			return IsPressed;
		}
		bool Is_MouseButtonPressed( MouseButton ButtonCode, bool Reset )
		{
			bool IsPressed;

			IsPressed = ( Mouse_Matrix[(int)ButtonCode] ? true : false );
			if( Reset ) Mouse_Matrix[(int)ButtonCode] = false;
			return IsPressed;
		}
		...
		public bool KeyDown( Key KeySym )
		{
			...
			switch( KeySym )
			{
				...
				case Key.C: { Mouse_Matrix[1] = true; Actor.Action_MouseButtonClick( 0 ); break; }
				case Key.V: { Mouse_Matrix[2] = true; Actor.Action_MouseButtonClick( 1 ); break; }
				case Key.B: { Mouse_Matrix[3] = true; Actor.Action_MouseButtonClick( 2 ); break; }
				case Key.X: { Mouse_Matrix[4] = true; Actor.Action_MouseButtonClick( 3 ); break; }
				case Key.N: { Mouse_Matrix[5] = true; Actor.Action_MouseButtonClick( 4 ); break; }

				// This will Save/Load player position and state (Debug only).

				case Key.P:
					if( VoxelGlobalSettings.COMPILEOPTION_DEBUGFACILITY )
					{
						if( Keyboard_Matrix[(int)Key.ShiftLeft] != 0 )
						{
#if asdfasdf
				...
				case Key.U: GameEnv.Stop_Programmable_Robots = true; break;
				...
				case Key.F10:
#if asdfasdf
					if( Keyboard_Matrix[(int)Key.ShiftLeft] )
					{
		...
		public bool MouseButtonRelease( MouseButton nButton, float Absolute_x, float Absolute_y )
		{
			...
			Mouse_Matrix[(int)nButton] = false;
			...
			{
				{
					...
					if( Actor != null )
					{
						Actor.Action_MouseButtonRelease( (uint)nButton - 1 );
					}
				}
			}
			return ( true );
