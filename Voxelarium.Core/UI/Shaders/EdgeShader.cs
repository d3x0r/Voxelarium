...
namespace Voxelarium.Core.UI.Shaders
{
	internal class SimpleEdgeShader : Shader
	{
		...
		const string Fragment_Simple =
			...
			+ "  gl_FragColor = vec4( h * white + (g * in_Color.rgb), in_Color.a ) ;"
#endif
			+ "}";
		...
		internal void DrawQuad( Vector3[] verts, Vector2[] texture, ref Vector4 color , float power = 400.0f )	
		{
	...
	internal class ColorEdgeShader : Shader
	{
		...
		internal void DrawQuad( Vector3[] verts, Vector2[] texture, Vector4[] colors, float power = 400.0f )
		{
			GL.Uniform1( power_id, power );
			Display.CheckErr();
			GL.EnableVertexAttribArray( vertex_attrib_id );
