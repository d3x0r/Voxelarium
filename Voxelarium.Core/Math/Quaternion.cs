...
namespace Voxelarium.LinearMath
{
	...
	*/
	...
	public struct btQuaternion
	{
		...
		//		explicit Quaternion(stringbtScalar v) : Tuple4<float>(v) {}
		...
		btQuaternion( float yaw, float pitch, float roll )
		{
			float halfYaw = yaw * btScalar.BT_HALF;
			float halfPitch = pitch * btScalar.BT_HALF;
			float halfRoll = roll * btScalar.BT_HALF;
			float cosYaw = btScalar.btCos( halfYaw );
			float sinYaw = btScalar.btSin( halfYaw );
			float cosPitch = btScalar.btCos( halfPitch );
			float sinPitch = btScalar.btSin( halfPitch );
			float cosRoll = btScalar.btCos( halfRoll );
			float sinRoll = btScalar.btSin( halfRoll );
		...
		public static void Mult( ref btQuaternion q, float s, out btQuaternion result )
		{
			result.x = q.x * s;
			result.y = q.y * s;
			result.z = q.z * s;
		...
		public static void Mult( ref btQuaternion q, float s, out btVector3 result )
		{
			result.x = q.x * s;
			result.y = q.y * s;
			result.z = q.z * s;
		...
		public void Mult( float s, out btVector3 result )
		{
			result.x = x * s;
			result.y = y * s;
			result.z = z * s;
		...
		/*@brief Return the dot product between this quaternion and another
		  @param q The other quaternion */
		public float dot( ref btQuaternion q )
		{
			return x * q.x +
					y * q.y +
					z * q.z +
		...
		public void Mult( float s, out btQuaternion result )
		{
			result.x = x * s;
			result.y = y * s;
			result.z = z * s;
		...
		{
			float s = btScalar.btSqrt( length2() * q.length2() );
			//Debug.Assert( s != 0.0 );
		...
		/*@brief Return the angle between this quaternion and the other along the shortest path
		 @param q The other quaternion */
		public float angleShortestPath( ref btQuaternion q )
		{
			float s = btScalar.btSqrt( length2() * q.length2() );
			//Debug.Assert( s != 0.0 );
			if( dot( ref q ) < 0 ) // Take care of long angle case see http://en.wikipedia.org/wiki/Slerp
			{
				btQuaternion b;
				q.inverse( out b );
				return btScalar.btAcos( dot( ref b ) / s ) * btScalar.BT_TWO;
			}
			else
				return btScalar.btAcos( dot( ref q ) / s ) * btScalar.BT_TWO;
		}

		/*@brief Return the angle of rotation represented by this quaternion */
		public float getAngle()
		{
			float s = 2 * btScalar.btAcos( w );
			return s;
		}
		...
		public float getAngleShortestPath()
		{
			float s;
			if( dot( ref this ) < 0 )
				s = btScalar.BT_TWO * btScalar.btAcos( w );
			else
				s = btScalar.BT_TWO * btScalar.btAcos( -w );
		...
		{
			float s_squared = btScalar.BT_ONE - w * w;

			if( s_squared < 10 * btScalar.SIMD_EPSILON ) //Check for divide by zero
			{
				result.x = btScalar.BT_ONE; result.y = 0; result.z = 0; result.w = 0;  // Arbitrary
				return;
			}
			float s = btScalar.BT_ONE / btScalar.btSqrt( s_squared );
			Mult( s, out result );
		}
...
*/
		...
		public static float dot( ref btQuaternion q1, ref btQuaternion q2 )
		{
		...
		public float length( ref btQuaternion q )
		{
			return q.length();
		}

		/*@brief Return the angle between two quaternions*/
		public static float btAngle( ref btQuaternion q1, ref btQuaternion q2 )
		{
		...
		public void slerp( ref btQuaternion q1, ref btQuaternion q2, float t, out btQuaternion result )
		{
		...
		public void shortestArcQuat( ref btVector3 v0, ref btVector3 v1, out btQuaternion result ) // Game Programming Gems 2.10 make sure v0,v1 are normalized
		{
			...
			if( d < -btScalar.BT_ONE + btScalar.SIMD_EPSILON )
			{
				btVector3 n, unused;
			...
			float s = btScalar.btSqrt( ( 1.0f + d ) * 2.0f );
			float rs = 1.0f / s;
			result.x = c.x * rs;
			result.y = c.y * rs;
			result.z = c.z * rs;
