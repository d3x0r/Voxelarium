...
namespace Bullet.LinearMath
{
    ...
    */
	...
	public struct btVector3 : btIVector3
	{
		...
		public static btVector3 Max = new btVector3( btScalar.BT_MAX_FLOAT );
		public static btVector3 Min = new btVector3( btScalar.BT_MIN_FLOAT );
		...
		public btVector3( float x, float y, float z, float w )
		{
		...
		public void AddScale( ref btVector3 b, float s, out btVector3 result )
		{
			result.x = x + b.x * s;
			result.y = y + b.y * s;
			result.z = z + b.z * s;
		...
		public void SubScale( ref btVector3 b, float s, out btVector3 result )
		{
			result.x = x - b.x * s;
			result.y = y - b.y * s;
			result.z = z - b.z * s;
		...
		public void SubAndScale( ref btVector3 b, float s, out btVector3 result )
		{
			result.x = ( x - b.x ) * s;
			result.y = ( y - b.y ) * s;
			result.z = ( z - b.z ) * s;
		...
		public void Mult( float b, out btVector3 result )
		{
			result.x = x * b;
			result.y = y * b;
			result.z = z * b;
		...
		public void Div( float b, out btVector3 result )
		{
#if PARANOID_ASSERTS
			Debug.Assert( b != 0 );
#endif
		...
		public static float dot( ref btVector3 a, ref btVector3 b )
		{
			return a.x * b.x +
					a.y * b.y +
					a.z * b.z;
		...
		public static void setValue( out btVector3 result, float _x, float _y, float _z )
		{
			result.x = _x;
			result.y = _y;
			result.z = _z;
		...
		public static void setValue( out btVector3 result, float _x, float _y, float _z, float _w )
		{
			result.x = _x;
			result.y = _y;
			result.z = _z;
		...
		public float triple( ref btVector3 v1, ref btVector3 v2 )
		{
			return
		...
		public static void setInterpolate3( ref btVector3 v0, ref btVector3 v1, float rt, out btVector3 result )
		{
			float s = 1.0f - rt;
			result.x = s * v0.x + rt * v1.x;
			result.y = s * v0.y + rt * v1.y;
		...
		/*@brief Return the linear interpolation between this and another vector 
          @param v The other vector 
          @param t The ration of this to v (t = 0 => return this, t=1 => return other) */
		public void lerp( ref btVector3 v, float t, out btVector3 result )
		{
			result.x = x + ( v.x - x ) * t;
			result.y = y + ( v.y - y ) * t;
			result.z = z + ( v.z - z ) * t;
		...
		public static void Mult( float s, ref btVector3 v, out btVector3 result )
		{
			v.Mult( s, out result );
		}
		...
		public static float btDistance2( ref btVector3 v1, ref btVector3 v2 )
		{
		...
		public static float btDistance( ref btVector3 v1, ref btVector3 v2 )
		{
			return v1.distance( ref v2 );
		}

		/*@brief Return the angle between two vectors */
		public float btAngle( ref btVector3 v1, ref btVector3 v2 )
		{
		...
		public static float btTriple( ref btVector3 v1, ref btVector3 v2, ref btVector3 v3 )
		{
		...
		public static btVector3 operator *( btVector3 a, float b )
		{
		...
		{
			return "<" + x + "," + y + "," + z + "," + w + ">=" + btScalar.btSqrt( x * x + y * y + z * z + w * w );
		}
	...
	public struct btVector4
	{
		...
		{
			int maxIndex = -1;
			float maxVal = -btScalar.BT_LARGE_FLOAT;
			if( x > maxVal )
			{
				maxIndex = 0;
				maxVal = x;
		...
		{
			int minIndex = -1;
			float minVal = btScalar.BT_LARGE_FLOAT;
			if( x < minVal )
			{
				minIndex = 0;
				minVal = x;
