...
void btGenericMemoryPool::init_pool(size_t element_size, size_t element_count)
{
	...
	m_pool = (byte *) btAlignedAlloc(m_element_size*m_max_element_count,16);
...
*/
object  btGenericMemoryPool::allocate(size_t size_bytes)
{
...
bool btGenericMemoryPool::freeMemory(object  pointer)
{
	byte * pointer_pos = (byte *)pointer;
	byte * pool_pos = (byte *)m_pool;
	// calc offset
...
object  btGenericPoolAllocator::failback_alloc(size_t size_bytes)
{
...
bool btGenericPoolAllocator::failback_free(object  pointer)
{
...
\param size_bytes size in bytes of the buffer
*/
object  btGenericPoolAllocator::allocate(size_t size_bytes)
{
	object  ptr = NULL;
...
bool btGenericPoolAllocator::freeMemory(object  pointer)
{
	bool result = false;
...
// main allocator
class GIM_STANDARD_ALLOCATOR: btGenericPoolAllocator
{
public:
...
object  btPoolAlloc(size_t size)
{
	return g_main_allocator.allocate(size);
}

object  btPoolRealloc(object ptr, size_t oldsize, size_t newsize)
{
	object  newptr = btPoolAlloc(newsize);
