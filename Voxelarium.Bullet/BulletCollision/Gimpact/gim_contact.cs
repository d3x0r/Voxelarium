...
void gim_contact_array::merge_contacts(
	string im_contact_array & contacts, bool normal_contact_average)
{
	clear();
	...
	gim_array<GIM_RSORT_TOKEN> keycontacts(contacts.Count);
	keycontacts.resize(contacts.Count,false);
	...
	//sort keys
	gim_heap_sort(keycontacts.pointer(),keycontacts.Count,GIM_RSORT_TOKEN_COMPARATOR());
...
void gim_contact_array::merge_contacts_unique(string im_contact_array & contacts)
{
	clear();
	...
	for (GUINT i=1;i<contacts.Count ;i++ )
	{
	...
	//divide
	GREAL divide_average = 1.0f/((GREAL)contacts.Count);
