...
struct MyCallback : btTriangleRaycastCallback
		{
			int	m_ignorePart;
			int	m_ignoreTriangleIndex;
			

			MyCallback(ref btVector3 from, ref btVector3 to, int ignorePart, int ignoreTriangleIndex)
			...
			virtual double reportHit(ref btVector3 hitNormalLocal, double hitFraction, int partId, int triangleIndex)
			{
		...
		struct MyInternalTriangleIndexCallback : btInternalTriangleIndexCallback
		{
			btGImpactMeshShape*		m_gimpactShape;
			btCompoundShape*			m_colShape;
			double	m_depth;

			MyInternalTriangleIndexCallback (btCompoundShape* colShape, btGImpactMeshShape* meshShape, double depth)
			:m_colShape(colShape),
...
btCompoundShape*	btCreateCompoundFromGimpactShape(btGImpactMeshShape* gimpactMesh, double depth)
{
