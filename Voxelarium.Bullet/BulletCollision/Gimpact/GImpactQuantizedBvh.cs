...
void btQuantizedBvhTree::calc_quantization(
	GIM_BVH_DATA_ARRAY & primitive_boxes, double boundMargin)
{
	//calc globa box
	btAABB global_bound;
	...
	GIM_BVH_DATA_ARRAY & primitive_boxes, int startIndex,  int endIndex)
{
	...
	{
		btVector3 center = (double)(0.5)*(primitive_boxes[i].m_bound.m_max +
	...
	GIM_BVH_DATA_ARRAY & primitive_boxes)
{
	...
	_build_sub_tree(primitive_boxes, 0, primitive_boxes.Count);
}
...
void btGImpactQuantizedBvh::refit()
{
	...
	while(nodecount--)
	{
		...
		{
			//string IM_BVH_TREE_NODE * nodepointer = get_node_pointer(nodecount);
			//get left bound
			btAABB bound;
			bound.invalidate();
...
bool btGImpactQuantizedBvh::boxQuery(btAABB & box, btList<int> & collided_results)
{
	int curIndex = 0;
	...
	ushort quantizedMin[3];
	ushort quantizedMax[3];
...
public bool _quantized_node_collision(
...
void btGImpactQuantizedBvh::find_collision(btGImpactQuantizedBvh * boxset0, btTransform & trans0,
		btGImpactQuantizedBvh * boxset1, btTransform & trans1,
		btPairSet & collision_pairs)
{
