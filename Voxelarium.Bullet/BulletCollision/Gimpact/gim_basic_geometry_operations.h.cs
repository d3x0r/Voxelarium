#if ! GIM_BASIC_GEOMETRY_OPERATIONS_H_INCLUDED
...
public bool POINT_IN_HULL(
	string LASS_POINT& point,string LASS_PLANE * planes,GUINT plane_count)
{
	GREAL _dis;
...
template<typename CLASS_POINT,typename CLASS_PLANE>
public void PLANE_CLIP_SEGMENT(
	string LASS_POINT s1,
	string LASS_POINT &s2,string LASS_PLANE &plane,CLASS_POINT &clipped)
{
	GREAL _dis1,_dis2;
...
template<typename CLASS_POINT,typename CLASS_PLANE>
public eLINE_PLANE_INTERSECTION_TYPE PLANE_CLIP_SEGMENT2(
	string LASS_POINT& s1,
	string LASS_POINT &s2,
	string LASS_PLANE &plane,CLASS_POINT &clipped)
{
...
*/
template<typename CLASS_POINT,typename CLASS_PLANE>
public eLINE_PLANE_INTERSECTION_TYPE PLANE_CLIP_SEGMENT_CLOSEST(
	string LASS_POINT& s1,
	string LASS_POINT &s2,
	string LASS_PLANE &plane,
...
public bool RAY_PLANE_COLLISION(
	string LASS_PLANE & plane,
	string LASS_POINT & vDir,
	string LASS_POINT & vPoint,
...
*/
template<typename T,typename CLASS_POINT,typename CLASS_PLANE>
public GUINT LINE_PLANE_COLLISION(
	string LASS_PLANE & plane,
	string LASS_POINT & vDir,
	string LASS_POINT & vPoint,
	CLASS_POINT & pout,
...
*/
template<typename CLASS_POINT,typename CLASS_PLANE>
public bool INTERSECT_PLANES(
		string LASS_PLANE &p1,
		string LASS_PLANE &p2,
		CLASS_POINT &p,
		CLASS_POINT &d)
{
	VEC_CROSS(d,p1,p2);
...
public void CLOSEST_POINT_ON_SEGMENT(
	CLASS_POINT  cp, string LASS_POINT & v,
	string LASS_POINT &e1,string LASS_POINT &e2)
{
    vec3f _n;
    VEC_DIFF(_n,e2,e1);
    VEC_DIFF(cp,v,e1);
...
public bool LINE_INTERSECTION_PARAMS(
	string LASS_POINT & dir1,
	CLASS_POINT & point1,
	string LASS_POINT & dir2,
	CLASS_POINT &  point2,
	T& t1,T& t2)
{
    GREAL det;
...
template<typename CLASS_POINT>
public void SEGMENT_COLLISION(
	string LASS_POINT & vA1,
	string LASS_POINT & vA2,
	string LASS_POINT & vB1,
	string LASS_POINT & vB2,
	CLASS_POINT & vPointA,
	CLASS_POINT & vPointB)
{
    ...
    LINE_PLANE_COLLISION(_M,_AD,vA1,vPointA,_tp,(double)(0), (double)(1));
...
*/
template<typename T>
public bool BOX_AXIS_INTERSECT(T pos, T dir,T bmin, T bmax, T  tfirst, T & tlast)
{
	if(GIM_IS_ZERO(dir))
	{
...
//! Sorts 3 componets
template<typename T>
public void SORT_3_INDICES(
		string  * values,
		GUINT * order_indices)
{
	//get minimum
	order_indices = values[0] < values[1] ? (values[0] < values[2] ? 0 : 2) : (values[1] < values[2] ? 1 : 2);
