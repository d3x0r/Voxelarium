...
//!@{
	...
	inline string  * get_pointer_at(GUINT i)
	{
		return m_data + i;
	}
	...
	inline void push_back(string  & obj)
	{
	    this.growingCheck();
	    m_data[m_size] = obj;
	    m_size++;
	...
	inline void push_back_memcpy(string  & obj)
	{
	    this.growingCheck();
	...
	inline void insert(string  & obj,GUINT index)
	{
	    insert_mem(index);
	    m_data[index] = obj;
	}

	inline void resize(GUINT size, bool call_constructor = true, string & fillData=T())
	{
	    if(size>m_size)
	    {
            reserve(size);
