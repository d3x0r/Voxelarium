...
public:
	...
	*/
		...
		btVector3 * clip_points)
	{
		...
		EDGE_PLANE(tripoints,tripoints[1],tri_plane,edgeplane);
	...
	- 3: nearest edges
	*/
	public GUINT cross_line_intersection_test()
	{
		...
		string REAL midpoint_u = 0.5f*(isect_u[0]+isect_u[1]); // midpoint
		string REAL midpoint_v = 0.5f*(isect_v[0]+isect_v[1]); // midpoint
	...
	public bool triangle_collision(
					btVector3  u0,
					btVector3  u1,
					btVector3  u2,
					GREAL margin_u,
					...
					GIM_TRIANGLE_CONTACT_DATA  contacts)
	{
		...
		{
			if(du<0) //we need test behind the triangle plane
			{
		...
		{
			if(dv<0) //we need test behind the triangle plane
			{
		...
		else //clip V points against U
		{
			...
			contacts.m_separating_normal *= -1;
		}
...
public:
					...
					GIM_TRIANGLE_CONTACT_DATA  contacts)
	{
		...
		contacts2.m_separating_normal *= -1;
		...
		GIM_TRIANGLE_CONTACT_DATA & contact_data)
{
	...
	return calc_cache.triangle_collision(
					m_vertices,m_vertices[1],m_vertices[2],m_margin,
