#if ! GIM_TRI_COLLISION_H_INCLUDED
...
struct GIM_TRIANGLE_CONTACT_DATA
{
	...
	public void copy_from(string IM_TRIANGLE_CONTACT_DATA& other)
	{
	...
	GIM_TRIANGLE_CONTACT_DATA(string IM_TRIANGLE_CONTACT_DATA& other)
	{
		copy_from(other);
	}
    ...
    public void mergepoints_generic(string LASS_PLANE & plane,
    				GREAL margin, btVector3 * points, GUINT point_count, DISTANCE_FUNC distance_func)
    {	
    	m_point_count = 0;
	...
	//! classify points that are closer
	public void merge_points(btVector4 & plane, GREAL margin,
										 btVector3 * points, GUINT point_count)
	{
...
public:
    ...
    public GIM_AABB get_box()  const
    {
    	return GIM_AABB(m_vertices[0],m_vertices[1],m_vertices[2],m_margin);
    }

    public void get_normal(btVector3 normal)  const
    {
    	TRIANGLE_NORMAL(m_vertices[0],m_vertices[1],m_vertices[2],normal);
    }

    public void get_plane(btVector4 plane)  const
    {
    	TRIANGLE_PLANE(m_vertices,m_vertices[1],m_vertices[2],plane);;
    }

    public void apply_transform(btTransform  trans)
    {
    	m_vertices = trans(m_vertices[0]);
    	m_vertices[1] = trans(m_vertices[1]);
    	m_vertices[2] = trans(m_vertices[2]);
    }

    public void get_edge_plane(GUINT edge_index,btVector3 triangle_normal,btVector4 &plane)  const
    {
		btVector3  e0 = m_vertices[edge_index];
		btVector3  e1 = m_vertices[(edge_index+1)%3];
    ...
    */
    public void get_triangle_transform(btTransform & triangle_transform)  const
    {
	...
	*/
	...
	\
	*/
	public bool collide_triangle(
		string IM_TRIANGLE & other,
		GIM_TRIANGLE_CONTACT_DATA & contact_data)
	{
		//test box collisioin
		GIM_AABB boxu(m_vertices,m_vertices[1],m_vertices[2],m_margin);
...
if 0.0<= u+v <=1.0 then they are inside of triangle
	...
	*/
	public bool is_point_inside(btVector3  point, btVector3  tri_normal)
	{
		//Test with edge 0
	...
	public bool ray_collision(
		btVector3  vPoint,
		btVector3  vDir, btVector3  pout, btVector3  triangle_normal,
		GREAL & tparam, GREAL tmax = G_REAL_INFINITY)
	{
		...
		GUINT res = LINE_PLANE_COLLISION(faceplane,vDir,vPoint,pout,tparam, (double)(0), tmax);
	...
	public bool ray_collision_front_side(
		btVector3  vPoint,
		btVector3  vDir, btVector3  pout, btVector3  triangle_normal,
		GREAL & tparam, GREAL tmax = G_REAL_INFINITY)
	{
