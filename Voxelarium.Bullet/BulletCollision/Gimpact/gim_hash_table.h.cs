...
struct GIM_HASH_TABLE_NODE
{
    ...
    GIM_HASH_TABLE_NODE(string IM_HASH_TABLE_NODE & value)
    {
        m_key = value.m_key;
        m_data = value.m_data;
    }

    GIM_HASH_TABLE_NODE(GUINT key, string  & data)
    {
        m_key = key;
        m_data = data;
    }

    bool operator <(string IM_HASH_TABLE_NODE<T> & other)
	{
...
public:
	template<class T>
	inline GUINT operator()( string & a)
	{
		return a.m_key;
	}
};
...
public:
	template<class T>
	inline int operator() ( string & a, GUINT key)
	{
...
public:
	template<class T>
	inline int operator() ( string & a, string & b )
	{
...
static string UINT gim_prime_list[GIM_NUM_PRIME] =
{
...
protected:
    ...
    */
    ...
    inline void _rehash()
    {
        ...
        _node_type * nodesptr = m_nodes.pointer();
        for(GUINT i=0;i<(GUINT)m_nodes.Count;i++)
        {
            ...
            if(nodekey != GIM_INVALID_HASH)
            {
				...
				{//The new index is alreade used... discard this new incomming object, repeated key
				    Debug.Assert(m_hash_table[index]==nodekey);
    ...
    inline GUINT _assign_hash_table_cell(GUINT hashkey)
    {
        ...
        if(cell_index==GIM_INVALID_HASH)
        {
            ...
            GUINT cell_index = _find_avaliable_cell(hashkey);
            Debug.Assert(cell_index!=GIM_INVALID_HASH);
        }
        return cell_index;
    }
    ...
    {
        if(index >= m_nodes.Count) return false;
        if(m_nodes[index].m_key != GIM_INVALID_HASH)
        {
            ...
            Debug.Assert(cell_index!=GIM_INVALID_HASH);
            Debug.Assert(m_hash_table[cell_index]==index);
    ...
    */
    inline GUINT _insert_hash_table(GUINT hashkey, string  & value)
    {
        ...
        m_hash_table[cell_index] = m_nodes.Count;
    ...
    If so, the element has been inserted at the last position of the array.
    */
    inline GUINT _insert_hash_table_replace(GUINT hashkey, string  & value)
    {
        if(hashkey==GIM_INVALID_HASH)
        {
            //Insert anyway
    ...
    {
        if(index>=(GUINT)m_nodes.Count) return false;
        m_nodes.erase_sorted(index);
		if(m_nodes.Count<2) m_sorted = false;
        return true;
    }
    ...
    inline bool _erase_unsorted(GUINT index)
    {
        ...
        GUINT lastindex = m_nodes.Count-1;
    ...
    */
    inline void _insert_in_pos(GUINT hashkey, string  & value, GUINT pos)
    {
    ...
    inline GUINT _insert_sorted(GUINT hashkey, string  & value)
    {
        if(hashkey==GIM_INVALID_HASH || size()==0)
        {
            m_nodes.Add(_node_type(hashkey,value));
            return GIM_INVALID_HASH;
        }
        ...
        GUINT result_ind=0;
        GUINT last_index = m_nodes.Count-1;
    ...
    inline GUINT _insert_sorted_replace(GUINT hashkey, string  & value)
    {
        if(hashkey==GIM_INVALID_HASH || size()==0)
        {
            m_nodes.Add(_node_type(hashkey,value));
    ...
    inline GUINT  _insert_unsorted(GUINT hashkey, string  & value)
    {
        m_nodes.Add(_node_type(hashkey,value));
        m_sorted = false;
        return GIM_INVALID_HASH;
...
public:
    ...
    */
    ...
    bool sort()
    {
        ...
        if(m_nodes.Count<2) return false;


        _node_type * ptr = m_nodes.pointer();
        GUINT siz = m_nodes.Count;
    ...
    bool switch_to_hashtable()
    {
        ...
        if(m_nodes.Count<GIM_DEFAULT_HASH_TABLE_SIZE)
        {
            _resize_table(GIM_DEFAULT_HASH_TABLE_SIZE);
        }
        else
        {
            _resize_table(m_nodes.Count+1);
        }
    ...
    bool check_for_switching_to_hashtable()
    {
        ...
        if(!(m_nodes.Count< m_min_hash_table_size))
        {
            if(m_node_size == 0)
            {
    ...
    inline GUINT get_key(GUINT index)
    {
    ...
    */
    ...
    inline string & operator[](GUINT index)
    {
        return m_nodes[index].m_data;
    }
    ...
    */
    inline GUINT find(GUINT hashkey)
    {
		...
		GUINT last_index = m_nodes.Count;
        if(last_index<2)
        {
    ...
    */
    inline GUINT insert(GUINT hashkey, string  & element)
    {
        if(m_hash_table)
        {
    ...
    */
    inline GUINT insert_override(GUINT hashkey, string  & element)
    {
        if(m_hash_table)
        {
    ...
    /*!
    */
    inline GUINT insert_unsorted(GUINT hashkey,string  & element)
    {
        if(m_hash_table)
        {
            return this._insert_hash_table(hashkey,element);
