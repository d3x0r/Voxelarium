...
{
	if(item.links) item.links[0].links[1]=item.links[1]; else list=item.links[1];
...
{
	static btruct ZeroDummy : T {} zerodummy;
	value=zerodummy;
}
...
struct	btDbvtTreeCollider : btDbvt.ICollide
{
	...
	void	Process(btDbvtNode* na,btDbvtNode* nb)
	{
		if(na!=nb)
		{
...
//
btBroadphaseProxy*				btDbvtBroadphase::createProxy(	ref btVector3 aabbMin,
...
void	btDbvtBroadphase::getAabb(btBroadphaseProxy* absproxy,ref btVector3 aabbMin, ref btVector3 aabbMax )
{
	btDbvtProxy*						proxy=(btDbvtProxy*)absproxy;
...
void	btDbvtBroadphase::rayTest(ref btVector3 rayFrom,ref btVector3 rayTo, btBroadphaseRayCallback& rayCallback,ref btVector3 aabbMin,ref btVector3 aabbMax)
{
	BroadphaseRayTester callback(rayCallback);

	m_sets.rayTestInternal(	m_sets[0].m_root,
		rayFrom,
		rayTo,
...
void	btDbvtBroadphase::aabbTest(ref btVector3 aabbMin,ref btVector3 aabbMax,btBroadphaseAabbCallback& aabbCallback)
{
	BroadphaseAabbTester callback(aabbCallback);

	btTRIBUTE_ALIGNED16(btDbvtVolume)	bounds=btDbvtVolume::FromMM(aabbMin,aabbMax);
		//process all children, that overlap with  the given AABB bounds
	m_sets.collideTV(m_sets[0].m_root,bounds,callback);
...
void btDbvtBroadphase::performDeferredRemoval(btDispatcher* dispatcher)
{
	...
	if (m_paircache.hasDeferredRemoval())
	{
		...
		overlappingPairArray.quickSort(btBroadphasePairSortPredicate());
		overlappingPairArray.resize(overlappingPairArray.Count - invalidPair);
	}
}
...
*/
	...
	if(m_needcleanup)
	{
		...
		if(pairs.Count>0)
		{
			...
			int			ni=btMin(pairs.Count,btMax<int>(m_newpairs,(pairs.Count*m_cupdates)/100));
			for(int i=0;i<ni;++i)
			{
...
//
void							btDbvtBroadphase::getBroadphaseAabb(ref btVector3 aabbMin,ref btVector3 aabbMax)
{
