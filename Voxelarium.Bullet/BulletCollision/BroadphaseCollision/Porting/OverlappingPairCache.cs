...
namespace Bullet.Collision.BroadPhase
{
	...
	internal partial class btHashedOverlappingPairCache : btOverlappingPairCache
	{
		internal static int gOverlappingPairs = 0;
			...
			{
				{
					...
					pair.m_algorithm = null;//.~btCollisionAlgorithm();
				}
			}
		}

		internal class CleanPairCallback : btOverlapCallback
		{
			...
			public bool processOverlap( ref btBroadphasePair pair )
			{
		...
		class RemovePairCallback : btOverlapCallback
		{
			...
			public RemovePairCallback( btBroadphaseProxy obsoleteProxy )
			{
		...
		void growTables()
		{
			...
			if( m_hashTable.Count < newCapacity )
			{
				...
				for( i = 0; i < curHashtableSize; i++ )
				{
					...
					/*if (proxyId1 > proxyId2) 
						btSwap(proxyId1, proxyId2);*/
					int hashValue = static_cast<int>( getHash( static_cast<uint>( proxyId1 ), static_cast<uint>( proxyId2 ) ) & ( m_overlappingPairArray.capacity() - 1 ) );    // New hash value with new mask
		...
		btBroadphasePair internalAddPair( btBroadphaseProxy proxy0, btBroadphaseProxy proxy1 )
		{
			...
			int hash = static_cast<int>( getHash( static_cast<uint>( proxyId1 ), static_cast<uint>( proxyId2 ) ) & ( m_overlappingPairArray.capacity() - 1 ) ); // New hash value with new mask
			...
			/*for(int i=0;i<m_overlappingPairArray.Count;++i)
				...
				if(	(m_overlappingPairArray[i].m_pProxy0==proxy0)&&
					...
					{
					Console.WriteLine("Adding duplicated %u<>%u\r\n",proxyId1,proxyId2);
					internalFindPair(proxy0, proxy1, hash);
					}
				}*/
			int count = m_overlappingPairArray.Count;
			int oldCapacity = m_overlappingPairArray.capacity();
			object mem = &m_overlappingPairArray.expandNonInitializing();
			...
			if( oldCapacity < newCapacity )
			{
				...
				hash = static_cast<int>( getHash( static_cast<uint>( proxyId1 ), static_cast<uint>( proxyId2 ) ) & ( m_overlappingPairArray.capacity() - 1 ) );
			}
		...
		object removeOverlappingPair( btBroadphaseProxy proxy0, btBroadphaseProxy proxy1, btDispatcher dispatcher )
		{
			...
			int hash = static_cast<int>( getHash( static_cast<uint>( proxyId1 ), static_cast<uint>( proxyId2 ) )( m_overlappingPairArray.capacity() - 1 ) );
			...
			int pairIndex = int( pair - &m_overlappingPairArray );
			...
			int lastHash = static_cast<int>( getHash( static_cast<uint>( last.m_pProxy0.getUid() ), static_cast<uint>( last.m_pProxy1.getUid() ) ) & ( m_overlappingPairArray.capacity() - 1 ) );
		...
		void processAllOverlappingPairs( btOverlapCallback callback, btDispatcher dispatcher )
		{
			...
			//	Console.WriteLine("m_overlappingPairArray.Count=%d\n",m_overlappingPairArray.Count);
		...
		internal partial class btSortedOverlappingPairCache {
