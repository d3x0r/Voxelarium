...
bool	btCollisionWorldImporter::convertAllObjects( btBulletSerializedArrays* arrays)
{
	...
	for (i=0;i<arrays.m_bvhsDouble.Count;i++)
	{
	...
	for (i=0;i<arrays.m_bvhsFloat.Count;i++)
    {
        btOptimizedBvh* bvh = createOptimizedBvh();
	...
	for (i=0;i<arrays.m_colShapeData.Count;i++)
	{
		btCollisionShapeData* shapeData = arrays.m_colShapeData[i];
		btCollisionShape* shape = convertCollisionShape(shapeData);
		if (shape)
		{
	//		Console.WriteLine("shapeMap.insert(%x,%x)\n",shapeData,shape);
	...
	for (i=0;i<arrays.m_collisionObjectDataDouble.Count;i++)
	{
        ...
        if (shapePtr && *shapePtr)
        {
            ...
            colObjData.m_worldTransform.m_origin.m_floats[3] = 0;
            startTransform.deSerializeDouble(colObjData.m_worldTransform);

            btCollisionShape* shape = (btCollisionShape*)*shapePtr;
            btCollisionObject body = createCollisionObject(startTransform,shape,colObjData.m_name);
            body.setFriction((double)(colObjData.m_friction));
            body.setRestitution((double)(colObjData.m_restitution));
        ...
        {
            Console.WriteLine("error: no shape found\n");
        }
	}
	for (i=0;i<arrays.m_collisionObjectDataFloat.Count;i++)
	{
...
void btCollisionWorldImporter::deleteAllData()
{
	...
	for (i=0;i<m_allocatedCollisionObjects.Count;i++)
	{
		if(m_collisionWorld)
	...
	for (i=0;i<m_allocatedCollisionShapes.Count;i++)
	{
...
btCollisionShape* btCollisionWorldImporter::convertCollisionShape(  btCollisionShapeData* shapeData  )
{
	...
	case GIMPACT_SHAPE_PROXYTYPE:
		{
			...
			if (gimpactData.m_gimpactSubType == CONST_GIMPACT_TRIMESH_SHAPE)
			{
				...
				gimpactShape.setMargin((double)(gimpactData.m_collisionMargin));
				gimpactShape.updateBound();
				shape = gimpactShape;
			} else
			{
				Console.WriteLine("unsupported gimpact sub type\n");
			}
			break;
		}
#endif //SUPPORT_GIMPACT_SHAPE_IMPORT
	...
	//so deal with this
		...
		case CONVEX_HULL_SHAPE_PROXYTYPE:
			{
				...
				switch (shapeData.m_shapeType)
				{
					...
					case MULTI_SPHERE_SHAPE_PROXYTYPE:
						{
							...
							shape = createMultiSphereShape(tmpPos,radii,numSpheres);
							break;
						}
					case CONVEX_HULL_SHAPE_PROXYTYPE:
						{
					...
					default:
						{
							Console.WriteLine("error: cannot create shape type (%d)\n",shapeData.m_shapeType);
						}
				}
		...
		case TRIANGLE_MESH_SHAPE_PROXYTYPE:
		{
			...
			//Console.WriteLine("trimesh.m_collisionMargin=%f\n",trimesh.m_collisionMargin);
			break;
		}
		case COMPOUND_SHAPE_PROXYTYPE:
			{
				...
				btCompoundShapeChildData* childShapeDataArray = &compoundData.m_childShapePtr;


				List<btCollisionShape*> childShapes;
				for (int i=0;i<compoundData.m_numChildShapes;i++)
				{
					...
					{
#if _DEBUG
						Console.WriteLine("error: couldn't create childShape for compoundShape\n");
#endif
					}
		...
		default:
			{
#if _DEBUG
				Console.WriteLine("unsupported shape type (%d)\n",shapeData.m_shapeType);
#endif
			}
		}
...
char* btCollisionWorldImporter::duplicateName(string name)
{
	if (name)
	{
	...
	for (int i=0;i<meshData.m_numMeshParts;i++)
	{
		...
		} else
		{
			if (meshData.m_meshPartsPtr[i].m_3indices16)
			{
				...
				m_shortIndexArrays.Add(indexArray);
			...
			if (meshData.m_meshPartsPtr[i].m_3indices8)
			{
				...
				meshPart.m_triangleIndexStride = sizeof(byte)*3;

				string indexArray = (string btAlignedAlloc(sizeof(byte)*3*meshPart.m_numTriangles,16);
				m_charIndexArrays.Add(indexArray);
		...
		if (meshData.m_meshPartsPtr[i].m_vertices3f)
		{
			...
			m_floatVertexArrays.Add(vertices);
			...
			meshPart.m_vertexBase = (string)vertices;
		} else
		{
			...
			m_doubleVertexArrays.Add(vertices);
...
btStridingMeshInterfaceData* btCollisionWorldImporter::createStridingMeshInterfaceData(btStridingMeshInterfaceData* interfaceData)
{
	...
	m_allocatedbtStridingMeshInterfaceDatas.Add(newData);
...
/*
btRigidBody  btWorldImporter::createRigidBody(bool isDynamic, double mass, ref btTransform startTransform,btCollisionShape* shape,string bodyName)
{
	...
	m_allocatedRigidBodies.Add(body);
	return body;
...
btCollisionObject btCollisionWorldImporter::createCollisionObject(ref btTransform startTransform,btCollisionShape* shape, string bodyName)
{
	...
	m_allocatedCollisionObjects.Add(colObj);
...
btCollisionShape* btCollisionWorldImporter::createPlaneShape(ref btVector3 planeNormal,double planeConstant)
{
	btStaticPlaneShape* shape = new btStaticPlaneShape(planeNormal,planeConstant);
	m_allocatedCollisionShapes.Add(shape);
	return shape;
}
btCollisionShape* btCollisionWorldImporter::createBoxShape(ref btVector3 halfExtents)
{
	btBoxShape* shape = new btBoxShape(halfExtents);
	m_allocatedCollisionShapes.Add(shape);
	return shape;
}
btCollisionShape* btCollisionWorldImporter::createSphereShape(double radius)
{
...
btCollisionShape* btCollisionWorldImporter::createCapsuleShapeX(double radius, double height)
{
...
btCollisionShape* btCollisionWorldImporter::createCapsuleShapeY(double radius, double height)
{
...
btCollisionShape* btCollisionWorldImporter::createCapsuleShapeZ(double radius, double height)
{
...
btCollisionShape* btCollisionWorldImporter::createCylinderShapeX(double radius,double height)
{
...
btCollisionShape* btCollisionWorldImporter::createCylinderShapeY(double radius,double height)
{
...
btCollisionShape* btCollisionWorldImporter::createCylinderShapeZ(double radius,double height)
{
...
btCollisionShape* btCollisionWorldImporter::createConeShapeX(double radius,double height)
{
...
btCollisionShape* btCollisionWorldImporter::createConeShapeY(double radius,double height)
{
...
btCollisionShape* btCollisionWorldImporter::createConeShapeZ(double radius,double height)
{
...
btTriangleIndexVertexArray*	btCollisionWorldImporter::createTriangleMeshContainer()
{
	...
	m_allocatedTriangleIndexArrays.Add(in);
	return in;
}
...
btTriangleInfoMap* btCollisionWorldImporter::createTriangleInfoMap()
{
	...
	m_allocatedTriangleInfoMaps.Add(tim);
	return tim;
}
...
btBvhTriangleMeshShape* btCollisionWorldImporter::createBvhTriangleMeshShape(btStridingMeshInterface* trimesh, btOptimizedBvh* bvh)
{
	if (bvh)
	{
		...
		m_allocatedCollisionShapes.Add(bvhTriMesh);
		return bvhTriMesh;
	}

	btBvhTriangleMeshShape* ts = new btBvhTriangleMeshShape(trimesh,true);
	m_allocatedCollisionShapes.Add(ts);
	return ts;
...
btScaledBvhTriangleMeshShape* btCollisionWorldImporter::createScaledTrangleMeshShape(btBvhTriangleMeshShape* meshShape,ref btVector3 localScaling)
{
...
btMultiSphereShape* btCollisionWorldImporter::createMultiSphereShape(btVector3* positions,double* radi,int numSpheres)
{
...
{
	return m_allocatedCollisionShapes.Count;
}
...
btCollisionShape* btCollisionWorldImporter::getCollisionShapeByName(string name)
{
...
string	btCollisionWorldImporter::getNameForPointer(object ptr)
{
	string  namePtr = m_objectNameMap.find(ptr);
	if (namePtr && *namePtr)
		return *namePtr;
	return 0;
}
...
{
	return m_allocatedRigidBodies.Count;
}
...
{
	return m_allocatedBvhs.Count;
}
 btOptimizedBvh* btCollisionWorldImporter::getBvhByIndex(int index)
{
...
{
	return m_allocatedTriangleInfoMaps.Count;
}

btTriangleInfoMap* btCollisionWorldImporter::getTriangleInfoMapByIndex(int index)
{
