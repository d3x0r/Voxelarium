...
btSimplePair* btHashedSimplePairCache::findPair(int indexA, int indexB)
{
	...
	int hash = static_cast<int>(getHash(static_cast<uint>(indexA), static_cast<uint>(indexB)) & (m_overlappingPairArray.capacity()-1));
...
void	btHashedSimplePairCache::growTables()
{
	...
	if (m_hashTable.Count < newCapacity)
	{
		...
		for(i=0;i<curHashtableSize;i++)
		{
			...
			int	hashValue = static_cast<int>(getHash(static_cast<uint>(indexA),static_cast<uint>(indexB)) & (m_overlappingPairArray.capacity()-1));	// New hash value with new mask
...
btSimplePair* btHashedSimplePairCache::internalAddPair(int indexA, int indexB)
{
	...
	int	hash = static_cast<int>(getHash(static_cast<uint>(indexA),static_cast<uint>(indexB)) & (m_overlappingPairArray.capacity()-1));	// New hash value with new mask
	...
	if (oldCapacity < newCapacity)
	{
		...
		hash = static_cast<int>(getHash(static_cast<uint>(indexA),static_cast<uint>(indexB)) & (m_overlappingPairArray.capacity()-1));
	}
...
object btHashedSimplePairCache::removeOverlappingPair(int indexA, int indexB)
{
	gRemoveSimplePairs++;
	...
	int lastHash = static_cast<int>(getHash(static_cast<uint>(last.m_indexA), static_cast<uint>(last.m_indexB)) & (m_overlappingPairArray.capacity()-1));
