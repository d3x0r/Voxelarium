...
double resolveSingleCollision(
        btRigidBody body1,
        btCollisionObject colObj2,
		ref btVector3 contactPositionWorld,
		...
		double distance)
{
    ...
    ref btVector3 normal = contactNormalOnB;
    ...
    double velocityError = -(1.0f + restitution) * rel_vel;// * damping;
	double denom0 = body1.computeImpulseDenominator(contactPositionWorld,normal);
	double denom1 = body2? body2.computeImpulseDenominator(contactPositionWorld,normal) : 0;
	double relaxation = 1;
    ...
    normalImpulse = 0 > normalImpulse ? 0: normalImpulse;
                      ...
                      double distance, ref btVector3 normal,double impulse ,double timeStep)
{
	...
	Debug.Assert(btFabs(normalLenSqr) < (double)(1.1));
	if (normalLenSqr > (double)(1.1))
	{
		impulse = btScalar.BT_ZERO;
		return;
	}
	...
	double jacDiagABInv = btScalar.BT_ONE / jacDiagAB;
	...
	double contactDamping = (double)(0.2);

#if ONLY_USE_LINEAR_MASS
	double massTerm = btScalar.BT_ONE / (body1.getInvMass() + body2.getInvMass());
