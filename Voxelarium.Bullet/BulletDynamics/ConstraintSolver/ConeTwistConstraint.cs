...
#define CONETWIST_DEF_FIX_THRESH (double)(.05f)
...
btConeTwistConstraint::btConeTwistConstraint(btRigidBody rbA,ref btTransform rbAFrame)
											:btTypedConstraint(CONETWIST_CONSTRAINT_TYPE,rbA),m_rbAFrame(rbAFrame),
											 m_angularOnly(false),
											 m_useSolveConstraintObsolete(CONETWIST_USE_OBSOLETE_SOLVER)
{
	m_rbBFrame = m_rbAFrame;
	m_rbBFrame.setOrigin(btVector3(0., 0.,  0.0));
	init();	
}
...
void btConeTwistConstraint::init()
{
	...
	m_maxMotorImpulse = (double)(-1);

	setLimit((double)(BT_LARGE_FLOAT), (double)(BT_LARGE_FLOAT), (double)(BT_LARGE_FLOAT));
...
void btConeTwistConstraint::getInfo2NonVirtual (btConstraintInfo2* info,ref btTransform transA,ref btTransform transB,btMatrix3x3& invInertiaWorldA,btMatrix3x3& invInertiaWorldB)
{
    ...
    // set right hand side
	double linERP = (m_flags & BT_CONETWIST_FLAGS_LIN_ERP) ? m_linERP : info.erp;
	...
	if(m_solveSwingLimit)
	{
		...
		else
		{
			...
			J2[srow+1] = -ax1[1];
			J2[srow+2] = -ax1[2];
			double k = info.fps * m_biasFactor;
...
void	btConeTwistConstraint::buildJacobian()
{
	if (m_useSolveConstraintObsolete)
	{
		...
		m_accMotorImpulse = btVector3(0.,0., 0.0);
...
void	btConeTwistConstraint::solveConstraintObsolete(btSolverBody bodyA,btSolverBody bodyB,double	timeStep)
{
	#if ! __SPU__
	if (m_useSolveConstraintObsolete)
	{
		...
		if (!m_angularOnly)
		{
			...
			for (int i=0;i<3;i++)
			{		
				ref btVector3 normal = m_jac[i].m_linearJointAxis;
...
void	btConeTwistConstraint::updateRHS(double	timeStep)
{
	(void)timeStep;
...
void btConeTwistConstraint::calcAngleInfo()
{
	...
	double EllipseAngle = btFabs(swing1*swing1)* RMaxAngle1Sq + btFabs(swing2*swing2) * RMaxAngle2Sq;
	...
	if (m_twistSpan >= btScalar.BT_ZERO)
	{
		...
		double twist = btAtan2Fast( TwistRef.dot(b1Axis3), TwistRef.dot(b1Axis2) );
		m_twistAngle = twist;
...
void btConeTwistConstraint::calcAngleInfo2(ref btTransform transA, ref btTransform transB, btMatrix3x3& invInertiaWorldA,btMatrix3x3& invInertiaWorldB)
{
	m_swingCorrection = btScalar.BT_ZERO;
	m_twistLimitSign = btScalar.BT_ZERO;
	m_solveTwistLimit = false;
...

	{
		...
		if (m_swingSpan1 >= m_fixThresh && m_swingSpan2 >= m_fixThresh)
		{
			double swingAngle, swingLimit = 0; btVector3 swingAxis;
			...
			if (swingAngle > swingLimit * m_limitSoftness)
			{
				...
				// 1 == hard/real limit
				m_swingLimitRatio = 1;
				if (swingAngle < swingLimit && m_limitSoftness < 1 - SIMD_EPSILON)
				{
...
//					if(!btFuzzyZero(z))
				...
				target = x * ivA[0] + y * jvA[0] + z * kvA[0];
		...
		if (m_twistSpan >= (double)(0))
		{
			...
			if (m_twistAngle > m_twistSpan*m_limitSoftness)
			{
				...
				m_twistLimitRatio = 1;
				if (m_twistAngle < m_twistSpan && m_limitSoftness < 1 - SIMD_EPSILON)
				{
...
void btConeTwistConstraint::computeConeLimitInfo(btQuaternion& qCone,
												 double swingAngle, // out
												 ref btVector3 vSwingAxis, // out
												 double swingLimit) // out
{
	...
	if (swingAngle > SIMD_EPSILON)
	{
       ...
       Debug.Assert(fabs(vSwingAxis.x) <= SIMD_EPSILON));
#endif
		...
		double theta = atan2(fabs(vSwingAxis.y),fabs(vSwingAxis.z));
...
btVector3 btConeTwistConstraint::GetPointForAngle(double fAngleInRadians, double fLength)
{
	// compute x/y in ellipse using cone angle (0 . 2*PI along surface of cone)
	double xEllipse = btCos(fAngleInRadians);
	double yEllipse = btSin(fAngleInRadians);
...
void btConeTwistConstraint::computeTwistLimitInfo(btQuaternion& qTwist,
												  double twistAngle, // out
												  ref btVector3 vTwistAxis) // out
{
...
void btConeTwistConstraint::adjustSwingAxisToUseEllipseNormal(ref btVector3 vSwingAxis)
{
...
void btConeTwistConstraint::setMotorTarget(btQuaternion &q)
{
	//btTransform trACur = m_rbA.getCenterOfMassTransform();
	//btTransform trBCur = m_rbB.getCenterOfMassTransform();
...
void btConeTwistConstraint::setMotorTargetInConstraintSpace(btQuaternion &q)
{
	m_qTarget = q;
...
void btConeTwistConstraint::setParam(int num, double value, int axis)
{
	switch(num)
	{
...
double btConeTwistConstraint::getParam(int num, int axis) string {
	double retVal = 0;
	switch(num)
	{
		case BT_CONSTRAINT_ERP :
