...
btSliderConstraint::btSliderConstraint(btRigidBody rbA, btRigidBody rbB, ref btTransform frameInA, ref btTransform frameInB, bool useLinearReferenceFrameA)
...
btSliderConstraint::btSliderConstraint(btRigidBody rbB, ref btTransform frameInB, bool useLinearReferenceFrameA)
...
void btSliderConstraint::calculateTransforms(ref btTransform transA,ref btTransform transB)
{
...
void btSliderConstraint::testAngLimits(void)
{
	...
	{
		btVector3 axisA0 = m_calculatedTransformA.getBasis().getColumn(1);
		btVector3 axisA1 = m_calculatedTransformA.getBasis().getColumn(2);
		btVector3 axisB0 = m_calculatedTransformB.getBasis().getColumn(1);
//		double rot = btAtan2Fast(axisB0.dot(axisA1), axisB0.dot(axisA0));  
		double rot = btAtan2(axisB0.dot(axisA1), axisB0.dot(axisA0));  
...
{
	btVector3 ancorInA;
	ancorInA = m_realPivotAInW + (m_lowerLinLimit + m_upperLinLimit) * (double)(0.5) * m_sliderAxis;
...
void btSliderConstraint::getInfo2NonVirtual(btConstraintInfo2* info, ref btTransform transA,ref btTransform transB, ref btVector3 linVelA,ref btVector3 linVelB, double rbAinvMass,double rbBinvMass  )
{
	ref btTransform trA = getCalculatedTransformA();
	ref btTransform trB = getCalculatedTransformB();
...
//	double k = info.fps * info.erp * getSoftnessOrthoAng();
	double currERP = (m_flags  BT_SLIDER_FLAGS_ERP_ORTANG) ? m_softnessOrthoAng : m_softnessOrthoAng * info.erp;
	double k = info.fps * currERP;

	btVector3 u = ax1A.cross(ax1B);
	info.m_constraintError = k * u.dot(p);
	info.m_constraintError[s] = k * u.dot(q);
	if(m_flags  BT_SLIDER_FLAGS_CFM_ORTANG)
	{
		info.cfm = m_cfmOrthoAng;
	...
	if (limit || powered) 
	{
		...
		if(limit)
		{
			...
			double bounce = btFabs((double)(1.0) - getDampingLimLin());
	...
	if(limit || powered) 
	{
		...
		if(limit)
		{
			...
			double bounce = btFabs((double)(1.0) - getDampingLimAng());
			if(bounce > (double)(0.0))
			{
...
void btSliderConstraint::setParam(int num, double value, int axis)
{
	switch(num)
	{
...
double btSliderConstraint::getParam(int num, int axis) string {
	double retVal(SIMD_INFINITY);
	switch(num)
	{
	case BT_CONSTRAINT_STOP_ERP :
		if(axis < 1)
