...
// upper > lower means limited
	...
	void testLimitValue(double test_value);
};
...
// upper > lower means limited
	...
	void testLimitValue(int limitIndex, double test_value);
};
...
public:
	...
	btVector3 getAxis(int axis_index) string  return m_calculatedAxis[axis_index]; }

	// Get the relative Euler angle ( btGeneric6DofSpring2Constraint::calculateTransforms() must be called previously )
	double getAngle(int axis_index) string  return m_calculatedAxisAngleDiff[axis_index]; }
	...
	double getRelativePivotPosition(int axis_index) string  return m_calculatedLinearDiff[axis_index]; }

	void setFrames(btTransform & frameA, btTransform & frameB);

	void setLinearLowerLimit(ref btVector3 linearLower) { m_linearLimits.m_lowerLimit = linearLower; }
	void getLinearLowerLimit(ref btVector3 linearLower) { linearLower = m_linearLimits.m_lowerLimit; }
	void setLinearUpperLimit(ref btVector3 linearUpper) { m_linearLimits.m_upperLimit = linearUpper; }
	void getLinearUpperLimit(ref btVector3 linearUpper) { linearUpper = m_linearLimits.m_upperLimit; }
	...
	void setAngularLowerLimitReversed(ref btVector3 angularLower)
	{
		for(int i = 0; i < 3; i++) 
...
public int btGeneric6DofSpring2Constraint::calculateSerializeBufferSize()
{
	return sizeof(btGeneric6DofSpring2ConstraintData2);
}

public string btGeneric6DofSpring2Constraint::serialize(object dataBuffer, btSerializer* serializer)
{
