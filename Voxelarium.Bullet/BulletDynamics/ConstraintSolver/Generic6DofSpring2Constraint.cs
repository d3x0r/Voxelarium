...
btGeneric6DofSpring2Constraint::btGeneric6DofSpring2Constraint(btRigidBody rbA, btRigidBody rbB, ref btTransform frameInA, ref btTransform frameInB, RotateOrder rotOrder)
...
btGeneric6DofSpring2Constraint::btGeneric6DofSpring2Constraint(btRigidBody rbB, ref btTransform frameInB, RotateOrder rotOrder)
...
double btGeneric6DofSpring2Constraint::btGetMatrixElem(btMatrix3x3& mat, int index)
{
	int i = index%3;
	int j = index/3;
	return mat[i][j];
...
bool btGeneric6DofSpring2Constraint::matrixToEulerXYZ(btMatrix3x3 mat,ref btVector3 xyz)
{
	// rot =  cy*cz          -cy*sz           sy
	...
	if (fi < (double)(1.0f))
	{
		if (fi > (double)(-1.0f))
		{
			xyz = btAtan2(-btGetMatrixElem(mat,5),btGetMatrixElem(mat,8));
...
bool btGeneric6DofSpring2Constraint::matrixToEulerXZY(btMatrix3x3 mat,ref btVector3 xyz)
{
	// rot =  cy*cz          -sz           sy*cz
	//        cy*cx*sz+sx*sy  cx*cz        sy*cx*sz-cy*sx
	//        cy*sx*sz-cx*sy  sx*cz        sy*sx*sz+cx*cy

	double fi = btGetMatrixElem(mat,1);
	if (fi < (double)(1.0f))
	{
		if (fi > (double)(-1.0f))
		{
			xyz = btAtan2(btGetMatrixElem(mat,7),btGetMatrixElem(mat,4));
...
bool btGeneric6DofSpring2Constraint::matrixToEulerYXZ(btMatrix3x3 mat,ref btVector3 xyz)
{
	// rot =  cy*cz+sy*sx*sz  cz*sy*sx-cy*sz  cx*sy
	//        cx*sz           cx*cz           -sx
	//        cy*sx*sz-cz*sy  sy*sz+cy*cz*sx  cy*cx

	double fi = btGetMatrixElem(mat,5);
	if (fi < (double)(1.0f))
	{
		if (fi > (double)(-1.0f))
		{
			xyz = btAsin(-btGetMatrixElem(mat,5));
...
bool btGeneric6DofSpring2Constraint::matrixToEulerYZX(btMatrix3x3 mat,ref btVector3 xyz)
{
	// rot =  cy*cz   sy*sx-cy*cx*sz   cx*sy+cy*sz*sx
	//        sz           cz*cx           -cz*sx
	//        -cz*sy  cy*sx+cx*sy*sz   cy*cx-sy*sz*sx

	double fi = btGetMatrixElem(mat,3);
	if (fi < (double)(1.0f))
	{
		if (fi > (double)(-1.0f))
		{
			xyz = btAtan2(-btGetMatrixElem(mat,5),btGetMatrixElem(mat,4));
...
bool btGeneric6DofSpring2Constraint::matrixToEulerZXY(btMatrix3x3 mat,ref btVector3 xyz)
{
	// rot =  cz*cy-sz*sx*sy    -cx*sz   cz*sy+cy*sz*sx
	//        cy*sz+cz*sx*sy     cz*cx   sz*sy-cz*xy*sx
	//        -cx*sy              sx     cx*cy

	double fi = btGetMatrixElem(mat,7);
	if (fi < (double)(1.0f))
	{
		if (fi > (double)(-1.0f))
		{
			xyz = btAsin(btGetMatrixElem(mat,7));
...
bool btGeneric6DofSpring2Constraint::matrixToEulerZYX(btMatrix3x3 mat,ref btVector3 xyz)
{
	// rot =  cz*cy   cz*sy*sx-cx*sz   sz*sx+cz*cx*sy
	//        cy*sz   cz*cx+sz*sy*sx   cx*sz*sy-cz*sx
	//        -sy          cy*sx         cy*cx

	double fi = btGetMatrixElem(mat,6);
	if (fi < (double)(1.0f))
	{
		if (fi > (double)(-1.0f))
		{
			xyz = btAtan2(btGetMatrixElem(mat,7), btGetMatrixElem(mat,8));
...
void btGeneric6DofSpring2Constraint::calculateTransforms(ref btTransform transA,ref btTransform transB)
{
...
{
	ref btTransform transA = m_rbA.getCenterOfMassTransform();
	ref btTransform transB = m_rbB.getCenterOfMassTransform();
	ref btVector3 linVelA = m_rbA.getLinearVelocity();
	ref btVector3 linVelB = m_rbB.getLinearVelocity();
	ref btVector3 angVelA = m_rbA.getAngularVelocity();
	ref btVector3 angVelB = m_rbB.getAngularVelocity();
...
int btGeneric6DofSpring2Constraint::setLinearLimits(btConstraintInfo2* info, int row, ref btTransform transA,ref btTransform transB,ref btVector3 linVelA,ref btVector3 linVelB,ref btVector3 angVelA,ref btVector3 angVelB)
{
	//solve linear limits
...
int btGeneric6DofSpring2Constraint::setAngularLimits(btConstraintInfo2 *info, int row_offset, ref btTransform transA,ref btTransform transB,ref btVector3 linVelA,ref btVector3 linVelB,ref btVector3 angVelA,ref btVector3 angVelB)
{
	int row = row_offset;
...
void btGeneric6DofSpring2Constraint::setFrames(ref btTransform frameA, ref btTransform frameB)
{
	m_frameInA = frameA;
	m_frameInB = frameB;
	buildJacobian();
...
void btGeneric6DofSpring2Constraint::calculateJacobi(btRotationalLimitMotor2 * limot, ref btTransform transA,ref btTransform transB, btConstraintInfo2 *info, int srow, ref btVector3 ax1, int rotational, int rotAllowed)
{
...
int btGeneric6DofSpring2Constraint::get_limit_motor_info2(
	...
	btConstraintInfo2 *info, int row, ref btVector3 ax1, int rotational,int rotAllowed)
{
	int count = 0;
	...
	if (limot.m_enableSpring)
	{
		...
		double dt = BT_ONE / info.fps;
		double kd = limot.m_springDamping;
		double ks = limot.m_springStiffness;
		double vel = rotational ? angVelA.dot(ax1) - angVelB.dot(ax1) : linVelA.dot(ax1) - linVelB.dot(ax1);
//		double erp = 0.1;
		double cfm = BT_ZERO;
		double mA = BT_ONE / m_rbA.getInvMass();
		double mB = BT_ONE / m_rbB.getInvMass();
		double m = mA > mB ? mB : mA;
		double angularfreq = sqrt(ks / m);


		//limit stiffness (the spring should not be sampled faster that the quarter of its angular frequency)
		if(limot.m_springStiffnessLimited && 0.25 < angularfreq * dt)
		{
			ks = BT_ONE / dt / dt / (double)(16.0) * m;
		}
...
void btGeneric6DofSpring2Constraint::setParam(int num, double value, int axis)
{
	if((axis >= 0) && (axis < 3))
	{
		switch(num)
...
double btGeneric6DofSpring2Constraint::getParam(int num, int axis) string 
{
	double retVal = 0;
	if((axis >= 0) && (axis < 3))
	{
...
void btGeneric6DofSpring2Constraint::setAxis(ref btVector3 axis1,ref btVector3 axis2)
{
...
void btGeneric6DofSpring2Constraint::setBounce(int index, double bounce)
{
...
void btGeneric6DofSpring2Constraint::setTargetVelocity(int index, double velocity)
{
	Debug.Assert((index >= 0) && (index < 6));
	if (index<3)
...
void btGeneric6DofSpring2Constraint::setServoTarget(int index, double target)
{
	Debug.Assert((index >= 0) && (index < 6));
	if (index<3)
...
void btGeneric6DofSpring2Constraint::setMaxMotorForce(int index, double force)
{
	Debug.Assert((index >= 0) && (index < 6));
	if (index<3)
...
void btGeneric6DofSpring2Constraint::setStiffness(int index, double stiffness, bool limitIfNeeded)
{
	Debug.Assert((index >= 0) && (index < 6));
	if (index<3) {
...
void btGeneric6DofSpring2Constraint::setDamping(int index, double damping, bool limitIfNeeded)
{
	Debug.Assert((index >= 0) && (index < 6));
	if (index<3) {
...
void btGeneric6DofSpring2Constraint::setEquilibriumPoint(int index, double val)
{
	Debug.Assert((index >= 0) && (index < 6));
	if (index<3)
...
void btRotationalLimitMotor2::testLimitValue(double test_value)
{
...
void btTranslationalLimitMotor2::testLimitValue(int limitIndex, double test_value)
{
