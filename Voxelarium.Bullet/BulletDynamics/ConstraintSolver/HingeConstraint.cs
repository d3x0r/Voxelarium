...
btHingeConstraint::btHingeConstraint(btRigidBody rbA,btRigidBody rbB, ref btVector3 pivotInA,ref btVector3 pivotInB,
									 ref btVector3 axisInA,ref btVector3 axisInB, bool useReferenceFrameA)
...
btHingeConstraint::btHingeConstraint(btRigidBody rbA,ref btVector3 pivotInA,ref btVector3 axisInA, bool useReferenceFrameA)
...
btHingeConstraint::btHingeConstraint(btRigidBody rbA,btRigidBody rbB, 
								     ref btTransform rbAFrame, ref btTransform rbBFrame, bool useReferenceFrameA)
...
btHingeConstraint::btHingeConstraint(btRigidBody rbA, ref btTransform rbAFrame, bool useReferenceFrameA)
...
void	btHingeConstraint::buildJacobian()
{
	if (m_useSolveConstraintObsolete)
	{
		...
		if (!m_angularOnly)
		{
			...
			{
				normal[0].setValue((double)(1.0),0,0);
			}
		...
		new (m_jacAng)	btJacobianEntry(jointAxis0,
			m_rbA.getCenterOfMassTransform().getBasis().transpose(),
			m_rbB.getCenterOfMassTransform().getBasis().transpose(),
			m_rbA.getInvInertiaDiagLocal(),
			m_rbB.getInvInertiaDiagLocal());
...
static inline double btNormalizeAnglePositive(double angle)
{
  return btFmod(btFmod(angle, (double)(2.0*SIMD_PI)) + (double)(2.0*SIMD_PI), (double)(2.0*SIMD_PI));
}



static double btShortestAngularDistance(double accAngle, double curAngle)
{
	double result = btNormalizeAngle(btNormalizeAnglePositive(btNormalizeAnglePositive(curAngle) -
	btNormalizeAnglePositive(accAngle)));
	return result;
}

static double btShortestAngleUpdate(double accAngle, double curAngle)
{
	double tol(0.3);
...
double btHingeAccumulatedAngleConstraint::getAccumulatedHingeAngle()
{
...
void	btHingeAccumulatedAngleConstraint::setAccumulatedHingeAngle(double accAngle)
{
...
void	btHingeConstraint::getInfo2NonVirtual (btConstraintInfo2* info,ref btTransform transA,ref btTransform transB,ref btVector3 angVelA,ref btVector3 angVelB)
{
...
void btHingeConstraint::getInfo2Internal(btConstraintInfo2* info, ref btTransform transA,ref btTransform transB,ref btVector3 angVelA,ref btVector3 angVelB)
{
	...
	// linear RHS
	double normalErp = (m_flags & BT_HINGE_FLAGS_ERP_NORM) ? m_normalERP : info.erp;
...
void	btHingeConstraint::updateRHS(double	timeStep)
{
	(void)timeStep;
...
double btHingeConstraint::getHingeAngle()
{
	return getHingeAngle(m_rbA.getCenterOfMassTransform(),m_rbB.getCenterOfMassTransform());
}

double btHingeConstraint::getHingeAngle(ref btTransform transA,ref btTransform transB)
{
...
void btHingeConstraint::testLimit(ref btTransform transA,ref btTransform transB)
{
...
static btVector3 vHinge(0, 0, (double)(1));

void btHingeConstraint::setMotorTarget(btQuaternion& qAinB, double dt)
{
...
void btHingeConstraint::setMotorTarget(double targetAngle, double dt)
{
#if	_BT_USE_CENTER_LIMIT_
	...
	double curAngle  = getHingeAngle(m_rbA.getCenterOfMassTransform(),m_rbB.getCenterOfMassTransform());
...
void btHingeConstraint::getInfo2InternalUsingFrameOffset(btConstraintInfo2* info, ref btTransform transA,ref btTransform transB,ref btVector3 angVelA,ref btVector3 angVelB)
{
	Debug.Assert(!m_useSolveConstraintObsolete);
...
void btHingeConstraint::setParam(int num, double value, int axis)
{
	if((axis == -1) || (axis == 5))
	{
		switch(num)
...
double btHingeConstraint::getParam(int num, int axis) string {
	double retVal = 0;
	if((axis == -1) || (axis == 5))
	{
		switch(num)
