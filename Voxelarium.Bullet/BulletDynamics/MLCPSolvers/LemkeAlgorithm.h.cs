...
#if ! BT_NUMERICS_LEMKE_ALGORITHM_H_
#define BT_NUMERICS_LEMKE_ALGORITHM_H_

#include "LinearMath/btMatrixX.h"


#include <vector> //todo: replace by List
...
public:
  ...
  btLemkeAlgorithm(btMatrixXu& M_, btVectorXu& q_, int & DEBUGLEVEL_ = 0) :
   ...
   */
  void setSystem(btMatrixXu & M_, btVectorXu & q_)
	{
		m_M = M_;
		m_q = q_;
  }
   ...
   */
  btVectorXu solve(uint maxloops = 0);

  virtual ~btLemkeAlgorithm() {
  }

protected:
  int findLexicographicMinimum(btMatrixXu &A, int & pivotColIndex);
  bool LexicographicPositive(btVectorXu & v);
  void GaussJordanEliminationStep(btMatrixXu &A, int pivotRowIndex, int pivotColumnIndex, btAlignedObjectArray<int>& basis);
  bool greaterZero(btVectorXu & vector);
  bool validBasis(btAlignedObjectArray<int>& basis);
