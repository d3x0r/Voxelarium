...
struct btVectorX
{
	...
	void	setZero()
	{
		if (m_storage.Count)
		{
			...
			//		m_storage[i]=0;
			//memset(m_storage,0,sizeof(T)*m_storage.Count);
			btSetZero(m_storage,m_storage.Count);
		}
	}
	...
	{
		return m_storage.Count ? m_storage : 0;
	}
	
	string * getBufferPointer()
	{
		return m_storage.Count ? m_storage : 0;
	}
...
struct btMatrixX
{
	...
	List<T>	m_storage;
	btList< btList<int> > m_rowNonZeroElements1;
	...
	void resize(int rows, int cols)
	{
		...
		m_cols = cols;
		{
			CProfileSample sample = new CProfileSample("m_storage.resize");
	...
	*/
	...
	void copyLowerToUpperTriangle()
	{
		...
		//Console.WriteLine("copyLowerToUpperTriangle copied %d elements out of %dx%d=%d\n", count,rows(),cols(),cols()*rows());
	}
	...
	{
		{
			CProfileSample sample = new CProfileSample("storage=0");
			btSetZero(m_storage,m_storage.Count);
			//memset(m_storage,0,sizeof(T)*m_storage.Count);
			//for (int i=0;i<m_storage.Count;i++)
	//			m_storage[i]=0;
	...
	void	printMatrix(string msg)
	{
		...
		for (int i=0;i<rows();i++)
		{
			...
			{
				Console.WriteLine("%2.1f\t",(*this)(i,j));
			}
		}
		Console.WriteLine("\n---------------------\n");
	...
	btMatrixX operator*(btMatrixX other)
	{
		//btMatrixX*btMatrixX implementation, brute force
		Debug.Assert(cols() == other.rows());

		btMatrixX res(rows(),other.cols());
		res.setZero();
//		CProfileSample sample = new CProfileSample("btMatrixX mul");
...
//						bool useOtherCol = true;
	...
	void multiplyAdd2_p8r (double *B, double *C,  int numRows,  int numRowsOther ,int row, int col)
	{
		double *bb = B;
	...
	void multiply2_p8r (double *B, double *C,  int numRows,  int numRowsOther, int row, int col)
	{
		Debug.Assert (numRows>0 & numRowsOther>0 && B && C);
		double *bb = B;
		for ( int i = 0;i<numRows;i++)
		{
			double *cc = C;
	...
	void setSubMatrix(int rowstart,int colstart,int rowend,int colend,string  value)
	{
	...
	void setSubMatrix(int rowstart,int colstart,int rowend,int colend,btMatrixX& block)
	{
		Debug.Assert(rowend+1-rowstart == block.rows());
		Debug.Assert(colend+1-colstart == block.cols());
	...
	void setSubMatrix(int rowstart,int colstart,int rowend,int colend,btVectorX<T>& block)
	{
		Debug.Assert(rowend+1-rowstart == block.rows());
		Debug.Assert(colend+1-colstart == block.cols());
		for (int row=0;row<block.rows();row++)
...
template <typename T> 
std::ostream& operator<< (std::ostream& os, btVectorX<T>& mat)
	{
