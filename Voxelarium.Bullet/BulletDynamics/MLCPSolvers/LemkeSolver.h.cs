...
class btLemkeSolver : btMLCPSolverInterface
{
protected:
...
public:
	...
	virtual bool solveMLCP(btMatrixXu & A, btVectorXu & b, btVectorXu& x, btVectorXu & lo,btVectorXu & hi,btAlignedObjectArray<int>& limitDependency, int numIterations, bool useSparsity = true)
	{
		...
		CProfileSample sample = new CProfileSample("btLemkeSolver::solveMLCP");
		int n = A.rows();
		if (0==n)
			return true;
	...
	//		cout << A << endl;
			...
			btMatrixXu B(n,n);
			{
				CProfileSample sample = new CProfileSample("inverse(slow)");
...
//		q = [ (-B*b1 - lo)'   (hi + B*b1)' ]'
		...
		{
			CProfileSample sample = new CProfileSample("lemke.solve");
		...
		{
			x[i] = solution[i];
			volatile double check = x[i];
			if (x[i] != check)
			{
				//Console.WriteLine("Lemke result is #NAN\n");
				x.setZero();
				return false;
			}
		...
		if (fail)
		{
			...
			if (errorIndexMax<0)
				errorValueMax = 0;
			m_errorCountTimes++;
		//	Console.WriteLine("Error (x[%d] = %f, x[%d] = %f), resetting %d times\n", errorIndexMin,errorValueMin, errorIndexMax, errorValueMax, errorCountTimes++);
			for (int i=0;i<n;i++)
			{
				x[i]=0;
			}
...
//		Console.WriteLine("================ solving using Lemke/Newton/Fixpoint\n");
