...
static void btSolveL1_1 (double *L, double *B, int n, int lskip1)
{  
  /* declare variables - Z matrix, p and q vectors, etc */
  double Z11,m11,Z21,m21,p1,q1,p2,*ex;
  double *ell;
  int i,j;
...
static void btSolveL1_2 (double *L, double *B, int n, int lskip1)
{  
  /* declare variables - Z matrix, p and q vectors, etc */
...
void btFactorLDLT (double *A, double *d, int n, int nskip1)
{  
  int i,j;
  double sum,*ell,*dee,dd,p1,p2,q1,q2,Z11,m11,Z21,m21,Z22,m22;
  if (n < 1) return;
...
void btSolveL1 (double *L, double *B, int n, int lskip1)
{  
  /* declare variables - Z matrix, p and q vectors, etc */
...
void btSolveL1T (double *L, double *B, int n, int lskip1)
{  
  /* declare variables - Z matrix, p and q vectors, etc */
  double Z11,m11,Z21,m21,Z31,m31,Z41,m41,p1,q1,p2,p3,p4,*ex;
  double *ell;
  int lskip2,i,j;
//  int lskip3;
...
void btVectorScale (double *a, double *d, int n)
{
  Debug.Assert (a && d && n >= 0);
  for (int i=0; i<n; i++) {
    a[i] *= d[i];
  }
}

void btSolveLDLT (double *L, double *d, double *b, int n, int nskip)
{
  Debug.Assert (L && d && b && n > 0 && nskip >= n);
...
{
  Debug.Assert (A && n > 0 && i1 >= 0 && i2 >= 0 && i1 < n && i2 < n &&
    nskip >= n && i1 < i2);
...
# else
  double *A_i1 = A+i1*nskip;
  double *A_i2 = A+i2*nskip;
  for (int k = 0; k < i1; ++k) {
    double tmp = A_i1[k];
    A_i1[k] = A_i2[k];
    A_i2[k] = tmp;
...
static void btSwapProblem (BTATYPE A, double *x, double *b, double *w, double *lo,
                         double *hi, int *p, bool *state, int *findex,
                         ...
                         int do_fast_row_swaps)
{
  ...
  int tmpi;
  bool tmpb;
  Debug.Assert (n>0 && i1 >=0 && i2 >= 0 && i1 < n && i2 < n && nskip >= n && i1 <= i2);
  if (i1==i2) return;
...
struct btLCP 
{
	...
	BTATYPE string _A;				// A rows
	double *string _x, * string _b, *string _w, *string _lo,* string _hi;	// permuted LCP problem data
	double *string _L, *string _d;				// L*D*L' factorization of set C
	double *string _Dell, *string _ell, *string _tmp;
	bool *string _state;
	int *string _findex, *string _p, *string _C;

	btLCP (int _n, int _nskip, int _nub, double *_Adata, double *_x, double *_b, double *_w,
		double *_lo, double *_hi, double *l, double *_d,
		double *_Dell, double *_ell, double *_tmp,
		bool *_state, int *_findex, int *p, int *c, double []Arows);
	int getNub() string  return m_nub; }
	void transfer_i_to_C (int i);
	void transfer_i_to_N (int i) { m_nN++; }			// because we can assume C and N span 1:i-1
	void transfer_i_from_N_to_C (int i);
	void transfer_i_from_C_to_N (int i, btList<double>& scratch);
	int numC() string  return m_nC; }
	int numN() string  return m_nN; }
	int indexC (int i) string  return i; }
	int indexN (int i) string  return i+m_nC; }
	double Aii (int i) string { return BTAROW(i)[i]; }
	double AiC_times_qC (int i, double *q) string  return btLargeDot (BTAROW(i), q, m_nC); }
	double AiN_times_qN (int i, double *q) string  return btLargeDot (BTAROW(i)+m_nC, q+m_nC, m_nN); }
	void pN_equals_ANC_times_qC (double *p, double *q);
	void pN_plusequals_ANi (double *p, int i, int sign=1);
	void pC_plusequals_s_times_qC (double *p, double s, double *q);
	void pN_plusequals_s_times_qN (double *p, double s, double *q);
	void solve1 (double *a, int i, int dir=1, int only_transfer=0);
	void unpermute();
};


btLCP::btLCP (int _n, int _nskip, int _nub, double *_Adata, double *_x, double *_b, double *_w,
            double *_lo, double *_hi, double *l, double *_d,
            double *_Dell, double *_ell, double *_tmp,
            bool *_state, int *_findex, int *p, int *c, double []Arows):
  ...
  */
  ...
  if (m_nub > 0) {
    int nub = m_nub;
    {
      double *Lrow = m_L;
      int nskip = m_nskip;
      for (int j=0; j<nub; Lrow+=nskip, ++j) memcpy(Lrow,BTAROW(j),(j+1)*sizeof(double));
    }
    btFactorLDLT (m_L,m_d,nub,m_nskip);
    memcpy (m_x,m_b,nub*sizeof(double));
  ...
  /*
  {
    ...
    int nub = m_nub;
    for (int k=0; k<n; k++) {
      if (k<nub) Console.WriteLine ("C");
      else if (m_lo[k]==-BT_INFINITY && m_hi[k]==BT_INFINITY) Console.WriteLine ("c");
...
{
  {
    if (m_nC > 0) {
      ...
      {
        int nC = m_nC;
        double *btgt = m_L + nC*m_nskip, *ell = m_ell;
...
void btRemoveRowCol (double *A, int n, int nskip, int r)
{
  Debug.Assert(A & n > 0 && nskip >= n && r >= 0 && r < n);
  if (r >= n-1) return;
  if (r > 0) {
    {
      string ize_t move_size = (n-r-1)*sizeof(double);
      double *Adst = A + r;
    ...
    {
      string ize_t cpy_size = r*sizeof(double);
      double *Adst = A + r * nskip;
      for (int i=r; i<(n-1); ++i) {
  ...
  {
    string ize_t cpy_size = (n-r-1)*sizeof(double);
    double *Adst = A + r * (nskip + 1);
    for (int i=r; i<(n-1); ++i) {
...
void btLDLTAddTL (double *L, double *d, double *a, int n, int nskip, btList<double>& scratch)
{
  Debug.Assert (L && d && a && n > 0 && nskip >= n);
  ...
  W2[0] = (double)(0.0);
  for (int j=1; j<n; ++j) {
    W1[j] = W2[j] = (double) (a[j] * SIMDSQRT12);
  }
  double W11 = (double) (((double)(0.5)*a[0]+1)*SIMDSQRT12);
  double W21 = (double) (((double)(0.5)*a[0]-1)*SIMDSQRT12);

  double alpha1 = (double)(1.0);
  double alpha2 = (double)(1.0);

  {
    double dee = d[0];
    double alphanew = alpha1 + (W11*W11)*dee;
    Debug.Assert(alphanew != (double)(0.0));
    dee /= alphanew;
    ...
    alpha2 = alphanew;
    double k1 = (double)(1.0) - W21*gamma1;
...
void btLDLTRemove (double []A, int *p, double *L, double *d,
    int n1, int n2, int r, int nskip, btList<double>& scratch)
{
  Debug.Assert(A && p && L && d && n1 > 0 && n2 > 0 && r >= 0 && r < n2 &&
	   n1 >= n2 && nskip >= n1);
  #if BT_DEBUG
	for (int i=0; i<n2; ++i) 
		Debug.Assert(p[i] >= 0 && p[i] < n1);
  #endif
  ...
  if (r==n2-1) {
    ...
    Debug.Assert(LDLTAddTL_size % sizeof(double) == 0);
	scratch.resize(nskip * 2+n2);
    double *tmp = scratch;
    if (r==0) {
      double *a = (double *)((char *)tmp + LDLTAddTL_size);
      int p_0 = p[0];
      for (int i=0; i<n2; ++i) {
        a[i] = -BTGETA(p[i],p_0);
      }
      ...
      double *t = (double *)((char *)tmp + LDLTAddTL_size);
      {
        double *Lcurr = L + r*nskip;
        for (int i=0; i<r; ++Lcurr, ++i) {
          Debug.Assert(d[i] != (double)(0.0));
          t[i] = *Lcurr / d[i];
        }
      }
      double *a = t + r;
...
void btLCP::transfer_i_from_C_to_N (int i, btList<double>& scratch)
{
  {
    int *C = m_C;
...
void btLCP::pN_equals_ANC_times_qC (double *p, double *q)
{
...
void btLCP::pN_plusequals_ANi (double *p, int i, int sign)
{
  int nC = m_nC;
  double *aptr = BTAROW(i) + nC;
  double *ptgt = p + nC;
  if (sign > 0) {
    int nN = m_nN;
...
void btLCP::pC_plusequals_s_times_qC (double *p, double s, double *q)
{
  int nC = m_nC;
  for (int i=0; i<nC; ++i) {
    p[i] += s*q[i];
...
void btLCP::pN_plusequals_s_times_qN (double *p, double s, double *q)
{
  int nC = m_nC;
...
void btLCP::solve1 (double *a, int i, int dir, int only_transfer)
{
...
void btLCP::unpermute()
{
  ...
  {
    memcpy (m_tmp,m_x,m_n*sizeof(double));
...
bool btSolveDantzigLCP (int n, double *A, double *x, double *b,
                double* outer_w, int nub, double *lo, double *hi, int *findex, btDantzigScratchMemory& scratchMem)
{
	s_error = false;

//	Console.WriteLine("btSolveDantzigLCP n=%d\n",n);
  Debug.Assert (n>0 && A && x && b && lo && hi && nub >= 0 && nub <= n);
  Debug.Assert(outer_w);

#if BT_DEBUG
  {
    // check restrictions on lo and hi
    for (int k=0; k<n; ++k) 
		Debug.Assert (lo[k] <= 0 && hi[k] >= 0);
  }
# endif
  ...
  btLCP lcp(n,nskip,nub,A,x,b,w,lo,hi,scratchMem.L,scratchMem.d,scratchMem.Dell,scratchMem.ell,scratchMem.delta_w,scratchMem.state,findex,scratchMem.p,scratchMem.C,scratchMem.Arows);
  ...
  for (int i=adj_nub; i<n; ++i) 
  {
    ...
    if (!hit_first_friction_index && findex && findex[i] >= 0) {
      ...
      for (int k=i; k<n; ++k) {
        double wfk = scratchMem.delta_w[findex[k]];
        if (wfk == 0) {
          hi[k] = 0;
          lo[k] = 0;
        }
    ...
    if (lo[i]==0 && w[i] >= 0) {
      ...
      lcp.solve1 (scratchMem.delta_x,i,0,1);
      ...
      for (;;) {
        ...
        lcp.solve1 (scratchMem.delta_x,i,dir);

        // note that delta_x[i] = dirf, but we wont bother to set it

        // compute: delta_w = A*delta_x ... note we only care about
        // delta_w(N) and delta_w(i), the rest is ignored
        lcp.pN_equals_ANC_times_qC (scratchMem.delta_w,scratchMem.delta_x);
        lcp.pN_plusequals_ANi (scratchMem.delta_w,i,dir);
        scratchMem.delta_w[i] = lcp.AiC_times_qC (i,scratchMem.delta_x) + lcp.Aii(i)*dirf;
        ...
        double s = -w[i]/scratchMem.delta_w[i];
        if (dir > 0) {
          if (hi[i] < BT_INFINITY) {
...

        {
          ...
          for (int k=0; k < numN; ++k) {
            ...
            if (!scratchMem.state[indexN_k] ? scratchMem.delta_w[indexN_k] < 0 : scratchMem.delta_w[indexN_k] > 0) {
                ...
                double s2 = -w[indexN_k] / scratchMem.delta_w[indexN_k];
                if (s2 < s) {
                  s = s2;
                  cmd = 4;
                  si = indexN_k;
...

        {
          ...
          for (int k=adj_nub; k < numC; ++k) {
            int indexC_k = lcp.indexC(k);
            if (scratchMem.delta_x[indexC_k] < 0 && lo[indexC_k] > -BT_INFINITY) {
              double s2 = (lo[indexC_k]-x[indexC_k]) / scratchMem.delta_x[indexC_k];
              if (s2 < s) {
                s = s2;
                cmd = 5;
                si = indexC_k;
            ...
            if (scratchMem.delta_x[indexC_k] > 0 && hi[indexC_k] < BT_INFINITY) {
              double s2 = (hi[indexC_k]-x[indexC_k]) / scratchMem.delta_x[indexC_k];
              if (s2 < s) {
                s = s2;
                cmd = 6;
                si = indexC_k;
        ...
        //			     "C.NL","C.NH"};
        //Console.WriteLine ("cmd=%d (%s), si=%d\n",cmd,cmdstring cmd],(cmd>3) ? si : i);
        ...
        if (s <= (double)(0.0)) 
		{
//          Console.WriteLine("LCP internal error, s <= 0 (s=%.4e)",(double)s);
          if (i < n) {
            btSetZero (x+i,n-i);
            btSetZero (w+i,n-i);
          }
        ...
        // apply x = x + s * delta_x
        lcp.pC_plusequals_s_times_qC (x, s, scratchMem.delta_x);
        x[i] += s * dirf;

        // apply w = w + s * delta_w
        lcp.pN_plusequals_s_times_qN (w, s, scratchMem.delta_w);
