...
{
	Console.WriteLine("Path Error: %s\n",msg);
}
  void __stdcall MyWarning(Void *data, Char *msg)
{
	Console.WriteLine("Path Warning: %s\n",msg);
}
...
class btPathSolver : btMLCPSolverInterface
{
public:
	...
	virtual bool solveMLCP(btMatrixXu & A, btVectorXu & b, btVectorXu& x, btVectorXu & lo,btVectorXu & hi,btAlignedObjectArray<int>& limitDependency, int numIterations, bool useSparsity = true)
	{
		...
		int numNonZero = rowIndices.Count;
		List<double> zResult;
		...
		{
			upperBounds.Add(hi[i]);
			lowerBounds.Add(lo[i]);
			rhs.Add(-b[i]);
		}


		SimpleLCP(numVariables,numNonZero,rowIndices,colIndices,values,rhs,lowerBounds,upperBounds, status, &zResult);

		if (status != MCP_Solved)
		{
			static string gReturnMsgs[] = {
				"Invalid return",
			...
			Console.WriteLine("ERROR: The PATH MCP solver failed: %s\n", gReturnMsgs[(uint)status]);// << std::endl;
			Console.WriteLine("using Projected Gauss Seidel fallback\n");
