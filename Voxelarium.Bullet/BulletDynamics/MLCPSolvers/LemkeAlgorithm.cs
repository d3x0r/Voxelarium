...
//STL/std::vector replaced by List
...
double btMachEps()
{
	...
	static double machEps = btScalar.BT_ONE;
	if (!calculated)
	{
		do {
			machEps /= (double)(2.0);
		...
		while ((double)(1.0 + (machEps/(double)(2.0))) != (double)(1.0));
...
double btEpsRoot() {
  ...
  btVectorXu btLemkeAlgorithm::solve(uint maxloops /* = 0*/)
{
	...
	A.setSubMatrix(0, 2 * dim, dim - 1, 2 * dim, -1);
 ...
 //   q_ >> A(0, 2 * dim + 1, dim - 1, 2 * dim + 1);
	...
	int pivotRowIndex = -1;
	double minValue = 1e30f;
	bool greaterZero = true;
	for (int i=0;i<dim;i++)
	{
		double v =A(i,2*dim+1);
		if (v<minValue)
		{
			minValue=v;
			pivotRowIndex = i;
...
//        maxloops = UINT_MAX; //TODO: not a really nice way, problem is: maxloops should be 2^dim (=1<<dim), but this could exceed UINT_MAX and thus the result would be 0 and therefore the lemke algorithm wouldn't start but probably would find a solution within less then UINT_MAX steps. Therefore this constant is used as a upper border right now...
  ...
  int btLemkeAlgorithm::findLexicographicMinimum(btMatrixXu& A, int & pivotColIndex) {
	  int RowIndex = 0;
	  int dim = A.rows();
	  btList<btVectorXu> Rows;
	  for (int row = 0; row < dim; row++) 
	  {

		  btVectorXu vec(dim + 1);
		  vec.setZero();//, INIT,  0.0)
		  Rows.Add(vec);
  ...
  bool btLemkeAlgorithm::LexicographicPositive(btVectorXu & v)
{
    int i = 0;
  //  if (DEBUGLEVEL)
    //  cout << "v " << v << endl;

    while(i < v.Count-1 && fabs(v[i]) < btMachEps())
      i++;
    if (v[i] > 0)
      return true;
...
void btLemkeAlgorithm::GaussJordanEliminationStep(btMatrixXu& A, int pivotRowIndex, int pivotColumnIndex, btAlignedObjectArray<int>& basis) 
{
  ...
  bool btLemkeAlgorithm::greaterZero(btVectorXu & vector)
{
    bool isGreater = true;
  ...
  bool btLemkeAlgorithm::validBasis(btAlignedObjectArray<int>& basis) 
  {
    bool isValid = true;
    for (int i = 0; i < basis.Count; i++) {
      if (basis[i] >= basis.Count * 2) { //then z0 is in the base
        isValid = false;
        break;
      }
    }
