...
namespace Bullet.Dynamics
{
	...
	internal class InplaceSolverIslandCallback : btSimulationIslandManager.IslandCallback
	{
		...
		internal override void processIsland( btCollisionObject[] bodies, int numBodies, btPersistentManifold[] manifolds, int numManifolds, int islandId )
		{
			if( islandId < 0 )
			{
		...
		void processConstraints()
		{
			...
			m_solver.solveGroup( bodies, m_bodies.Count, manifold, m_manifolds.Count, constraints, 0, m_constraints.Count, m_solverInfo, m_debugDrawer, m_dispatcher );
			m_bodies.Count = ( 0 );
			m_manifolds.Count = ( 0 );
			m_constraints.Count = ( 0 );
	...
	public partial class btDiscreteDynamicsWorld : btCollisionWorld
	{
		...
		internal int stepSimulation( double timeStep, int maxSubSteps, double fixedTimeStep )
		{
...
# ifndef BT_NO_PROFILE
		...
		internal void addCollisionObject( btCollisionObject collisionObject, short int collisionFilterGroup, short int collisionFilterMask )
		{
			btCollisionWorld::addCollisionObject( collisionObject, collisionFilterGroup, collisionFilterMask );
		}

		internal void removeCollisionObject( btCollisionObject collisionObject )
		{
		...
		internal void addRigidBody( btRigidBody body, short group, short mask )
		{
			if( !body.isStaticOrKinematicObject() && !( body.getFlags() & BT_DISABLE_WORLD_GRAVITY ) )
			{
				body.setGravity( m_gravity );
		...
		internal void addConstraint( btTypedConstraint constraint, bool disableCollisionsBetweenLinkedBodies )
		{
		...
		internal void removeConstraint( btTypedConstraint constraint )
		{
...
internal int getNumConstraints()
{
	return int( m_constraints.Count );
}
internal btTypedConstraint getConstraint( int index )
{
