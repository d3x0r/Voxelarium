...
struct btMultibodyLink 
{
    ...
    btVector3 m_dVector;   // vector from the inboard joint pos to this link's COM. (local frame.)ant.
	...
	// "axis" = spatial joint axis (Mirtich Defn 9 p104). (expressed in local frame.)ant.
	...
	void setAxisTop(int dof, btVector3 axis) { m_axes[dof].m_topVec = axis; }
	void setAxisBottom(int dof, btVector3 axis) { m_axes[dof].m_bottomVec = axis; }
	void setAxisTop(int dof, double &x, double &y, double &z) { m_axes[dof].m_topVec.setValue(x, y, z); }
	void setAxisBottom(int dof, double &x, double &y, double &z) { m_axes[dof].m_bottomVec.setValue(x, y, z); }
	btVector3  getAxisTop(int dof) string  return m_axes[dof].m_topVec; }
	btVector3  getAxisBottom(int dof) string  return m_axes[dof].m_bottomVec; }
...
btVector3 m_appliedConstraintForce;    // In WORLD frame
	...
	string m_linkName;//m_linkName memory needs to be managed by the developer/user!
	string m_jointName;//m_jointName memory needs to be managed by the developer/user!
			...
			m_jointName(0)
	{
		...
		setAxisTop(0, 0., 0.,  0.0);
		setAxisBottom(0, 1., 0.,  0.0);
		...
		//		
		m_jointPos[0] = m_jointPos[1] = m_jointPos[2] = m_jointPos[4] = m_jointPos[5] = m_jointPos[6] = 0;
		m_jointPos[3] = 1;			//"quat.w"
		m_jointTorque[0] = m_jointTorque[1] = m_jointTorque[2] = m_jointTorque[3] = m_jointTorque[4] = m_jointTorque[5] = 0;
	...
	void updateCacheMultiDof(double *pq = 0)
	{
		double *pJointPos = (pq ? pq : m_jointPos);
