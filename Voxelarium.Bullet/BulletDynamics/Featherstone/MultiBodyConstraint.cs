...
void	btMultiBodyConstraint::applyDeltaVee(btMultiBodyJacobianData& data, double* delta_vee, double impulse, int velocityIndex, int ndof)
{
	for (int i = 0; i < ndof; ++i)
		data.m_deltaVelocities[velocityIndex+i] += delta_vee[i] * impulse;
}

double btMultiBodyConstraint::fillMultiBodyConstraint(	btMultiBodySolverConstraint& solverConstraint,
															...
															bool isFriction, double desiredVelocity, double cfmSlip)
{
	...
	if (multiBodyA)
	{
		...
		{
			solverConstraint.m_deltaVelAindex = data.m_deltaVelocities.Count;
			multiBodyA.setCompanionId(solverConstraint.m_deltaVelAindex);
			data.m_deltaVelocities.resize(data.m_deltaVelocities.Count+ndofA);
		} else
		{
			Debug.Assert(data.m_deltaVelocities.Count >= solverConstraint.m_deltaVelAindex+ndofA);
		}

		//determine jacobian of this 1D constraint in terms of multibodyA's degrees of freedom
		//resize..
		solverConstraint.m_jacAindex = data.m_jacobians.Count;
		data.m_jacobians.resize(data.m_jacobians.Count+ndofA);
		//copy/determine
		if(jacOrgA)
		{
		...
		{
			double* jac1=&data.m_jacobians[solverConstraint.m_jacAindex];
		...
		//resize..
		data.m_deltaVelocitiesUnitImpulse.resize(data.m_deltaVelocitiesUnitImpulse.Count+ndofA);		//=> each constraint row has the constrained tree dofs allocated in m_deltaVelocitiesUnitImpulse
		Debug.Assert(data.m_jacobians.Count == data.m_deltaVelocitiesUnitImpulse.Count);
		double* delta = &data.m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacAindex];
		//determine..
		if(multiBodyA.isMultiDof())
	...
	if (multiBodyB)
	{
		...
		{
			solverConstraint.m_deltaVelBindex = data.m_deltaVelocities.Count;
			multiBodyB.setCompanionId(solverConstraint.m_deltaVelBindex);
			data.m_deltaVelocities.resize(data.m_deltaVelocities.Count+ndofB);
		}

		//determine jacobian of this 1D constraint in terms of multibodyB's degrees of freedom
		//resize..
		solverConstraint.m_jacBindex = data.m_jacobians.Count;
		data.m_jacobians.resize(data.m_jacobians.Count+ndofB);
		//copy/determine..
		if(jacOrgB)
		{
		...
		//resize..
		data.m_deltaVelocitiesUnitImpulse.resize(data.m_deltaVelocitiesUnitImpulse.Count+ndofB);
		Debug.Assert(data.m_jacobians.Count == data.m_deltaVelocitiesUnitImpulse.Count);
		double* delta = &data.m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacBindex];
		//determine..
		if(multiBodyB.isMultiDof())
	...
	double penetration = isFriction? 0 : posError+infoGlobal.m_linearSlop;
	...
	int ndofB  = 0;
	{
		...
		{
			ndofA = (multiBodyA.isMultiDof() ? multiBodyA.getNumDofs() : multiBodyA.getNumLinks()) + 6;
			double* jacA = &data.m_jacobians[solverConstraint.m_jacAindex];
			for (int i = 0; i < ndofA ; ++i)
		...
		if (multiBodyB)
		{
			...
			double* jacB = &data.m_jacobians[solverConstraint.m_jacBindex];
			for (int i = 0; i < ndofB ; ++i)
		...
		solverConstraint.m_friction = 0;//cp.m_combinedFriction;
	}
	...
	if (infoGlobal.m_solverMode & SOLVER_USE_WARMSTARTING)
	{
		...
		if (solverConstraint.m_appliedImpulse)
		{
			if (multiBodyA)
			{
				...
				double* deltaV = &data.m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacAindex];
			...
			{
				double impulse = solverConstraint.m_appliedImpulse;
				double* deltaV = &data.m_deltaVelocitiesUnitImpulse[solverConstraint.m_jacBindex];
	...
	*/
...

	{
		...
		double	velocityError = desiredVelocity - rel_vel;// * damping;
