...
#if ! BT_FEATHERSTONE_LINK_COLLIDER_H
...
public:
	...
	virtual bool checkCollideWithOverride(string btCollisionObject co)
	{
		btMultiBodyLinkCollider* other = btMultiBodyLinkCollider::upcast(co);
		if (!other)
			return true;
		...
		if (m_link>=0)
		{
			btMultibodyLink& link = m_multiBody.getLink(this.m_link);
		...
		if (other.m_link>=0)
		{
			btMultibodyLink& otherLink = other.m_multiBody.getLink(other.m_link);
