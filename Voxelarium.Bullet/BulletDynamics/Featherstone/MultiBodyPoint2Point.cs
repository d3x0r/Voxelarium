...
#if ! BTMBP2PCONSTRAINT_BLOCK_ANGULAR_MOTION_TEST
	#define BTMBP2PCONSTRAINT_DIM 3
#else
	#define BTMBP2PCONSTRAINT_DIM 6
#endif

btMultiBodyPoint2Point::btMultiBodyPoint2Point(btMultiBody* body, int link, btRigidBody bodyB, ref btVector3 pivotInA, ref btVector3 pivotInB)
...
btMultiBodyPoint2Point::btMultiBodyPoint2Point(btMultiBody* bodyA, int linkA, btMultiBody* bodyB, int linkB, ref btVector3 pivotInA, ref btVector3 pivotInB)
...
int btMultiBodyPoint2Point::getIslandIdA()
{
	if (m_rigidBodyA)
...
int btMultiBodyPoint2Point::getIslandIdB()
{
	if (m_rigidBodyB)
...
int numDim = BTMBP2PCONSTRAINT_DIM;
	...
	constraintRow.m_orgDofIndex = i;
		...
		double posError = i < 3 ? (pivotAworld-pivotBworld).dot(contactNormalOnB) : 0;
    ...
    //see btPoint2PointConstraint::getInfo2NonVirtual
		...
		Debug.Assert(m_bodyA.isMultiDof());

		double* jac1 = jacobianA(i);
		btVector3 normalAng = i >= 3 ? contactNormalOnB : dummy;
		btVector3 normalLin = i < 3 ? contactNormalOnB : dummy;
...
void btMultiBodyPoint2Point::debugDraw(btIDebugDraw drawer)
{
	btTransform tr;
	tr.setIdentity();
