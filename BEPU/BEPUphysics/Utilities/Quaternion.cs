...
namespace BEPUutilities
{
    ...
    public struct Quaternion : IEquatable<Quaternion>
    {
        ...
        {
            float trace = r.M1.X + r.M2.Y + r.M3.Z;
#if !WINDOWS
            q = new Quaternion();
#endif
            if (trace >= 0)
            {
                ...
                var inverseS = 1 / S;
                q.W = 0.25f * S;
                q.X = (r.M2.Z - r.M3.Y) * inverseS;
                q.Y = (r.M3.X - r.M1.Z) * inverseS;
                q.Z = (r.M1.Y - r.M2.X) * inverseS;
            }
            else if ((r.M1.X > r.M2.Y) & (r.M1.X > r.M3.Z))
            {
                var S = (float)Math.Sqrt(1.0 + r.M1.X - r.M2.Y - r.M3.Z) * 2; // S=4*qx 
                var inverseS = 1 / S;
                q.W = (r.M2.Z - r.M3.Y) * inverseS;
                q.X = 0.25f * S;
                q.Y = (r.M2.X + r.M1.Y) * inverseS;
                q.Z = (r.M3.X + r.M1.Z) * inverseS;
            }
            else if (r.M2.Y > r.M3.Z)
            {
                var S = (float)Math.Sqrt(1.0 + r.M2.Y - r.M1.X - r.M3.Z) * 2; // S=4*qy
                var inverseS = 1 / S;
                q.W = (r.M3.X - r.M1.Z) * inverseS;
                q.X = (r.M2.X + r.M1.Y) * inverseS;
                q.Y = 0.25f * S;
                q.Z = (r.M3.Y + r.M2.Z) * inverseS;
            }
            else
            {
                var S = (float)Math.Sqrt(1.0 + r.M3.Z - r.M1.X - r.M2.Y) * 2; // S=4*qz
                var inverseS = 1 / S;
                q.W = (r.M1.Y - r.M2.X) * inverseS;
                q.X = (r.M3.X + r.M1.Z) * inverseS;
                q.Y = (r.M3.Y + r.M2.Z) * inverseS;
                q.Z = 0.25f * S;
            }
        }
