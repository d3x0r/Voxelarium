...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class BoxBoxCollider
    {
        ...
        public static bool AreBoxesColliding(BoxShape a, BoxShape b, ref RigidTransform transformA, ref RigidTransform transformB)
        {
            ...
            bR.M1.X = aO.M11 * bO.M11 + aO.M12 * bO.M12 + aO.M13 * bO.M13;
            bR.M1.Y = aO.M11 * bO.M21 + aO.M12 * bO.M22 + aO.M13 * bO.M23;
            bR.M1.Z = aO.M11 * bO.M31 + aO.M12 * bO.M32 + aO.M13 * bO.M33;
            Matrix3x3 absBR;
            //Epsilons are added to deal with near-parallel edges.
            absBR.M1.X = Math.Abs(bR.M1.X) + Toolbox.Epsilon;
            absBR.M1.Y = Math.Abs(bR.M1.Y) + Toolbox.Epsilon;
            absBR.M1.Z = Math.Abs(bR.M1.Z) + Toolbox.Epsilon;
            float tX = t.X;
            t.X = t.Dot( ref aO.M1 );

            //Test the axes defines by entity A's rotation matrix.
            //A.X
            float rb = bX * absBR.M1.X + bY * absBR.M1.Y + bZ * absBR.M1.Z;
            if (Math.Abs(t.X) > aX + rb)
                return false;
            bR.M2.X = aO.M21 * bO.M11 + aO.M22 * bO.M12 + aO.M23 * bO.M13;
            bR.M2.Y = aO.M21 * bO.M21 + aO.M22 * bO.M22 + aO.M23 * bO.M23;
            bR.M2.Z = aO.M21 * bO.M31 + aO.M22 * bO.M32 + aO.M23 * bO.M33;
            absBR.M2.X = Math.Abs(bR.M2.X) + Toolbox.Epsilon;
            absBR.M2.Y = Math.Abs(bR.M2.Y) + Toolbox.Epsilon;
            absBR.M2.Z = Math.Abs(bR.M2.Z) + Toolbox.Epsilon;
            float tY = t.Y;
            t.Y = tX * aO.M21 + t.Y * aO.M22 + t.Z * aO.M23;

            //A.Y
            rb = bX * absBR.M2.X + bY * absBR.M2.Y + bZ * absBR.M2.Z;
            if (Math.Abs(t.Y) > aY + rb)
                return false;

            bR.M3.X = aO.M31 * bO.M11 + aO.M32 * bO.M12 + aO.M33 * bO.M13;
            bR.M3.Y = aO.M31 * bO.M21 + aO.M32 * bO.M22 + aO.M33 * bO.M23;
            bR.M3.Z = aO.M31 * bO.M31 + aO.M32 * bO.M32 + aO.M33 * bO.M33;
            absBR.M3.X = Math.Abs(bR.M31) + Toolbox.Epsilon;
            absBR.M3.Y = Math.Abs(bR.M32) + Toolbox.Epsilon;
            absBR.M3.Z = Math.Abs(bR.M33) + Toolbox.Epsilon;
        ...
        public static bool AreBoxesColliding(BoxShape a, BoxShape b, ref RigidTransform transformA, ref RigidTransform transformB, out float separationDistance, out Vector3 separatingAxis)
        {
            ...
            //Test the axes defines by entity A's rotation matrix.
            //A.X
            float rarb = aX + bX * absBR.M1.X + bY * absBR.M1.Y + bZ * absBR.M1.Z;
            if (t.X > rarb)
            {
            ...
            //A.Y
            rarb = aY + bX * absBR.M2.X + bY * absBR.M2.Y + bZ * absBR.M2.Z;
            if (t.Y > rarb)
            {
            ...
            bR.M3.Y = aO.M31 * bO.M21 + aO.M32 * bO.M22 + aO.M33 * bO.M23;
            bR.M3.Z = aO.M31 * bO.M31 + aO.M32 * bO.M32 + aO.M33 * bO.M33;
            absBR.M3.X = Math.Abs(bR.M3.X) + Toolbox.Epsilon;
            absBR.M3.Y = Math.Abs(bR.M3.Y) + Toolbox.Epsilon;
            absBR.M3.Z = Math.Abs(bR.M3.Z) + Toolbox.Epsilon;
            t.Z = tX * aO.M31 + tY * aO.M32 + t.Z * aO.M33;
        ...
        public static bool AreBoxesCollidingWithPenetration(BoxShape a, BoxShape b, ref RigidTransform transformA, ref RigidTransform transformB, out float distance, out Vector3 axis)
        {
            ...
            Matrix3x3 absBR;
            //Epsilons are added to deal with near-parallel edges.
            absBR.M1.X = Math.Abs(bR.M1.Y) + Toolbox.Epsilon;
            absBR.M1.Y = Math.Abs(bR.M1.X) + Toolbox.Epsilon;
            absBR.M1.Z = Math.Abs(bR.M1.Z) + Toolbox.Epsilon;
            float tX = t.X;
            t.X = t.X * aO.M11 + t.Y * aO.M12 + t.Z * aO.M13;
...
#endif
        {
			Vector3 aV = new Vector3( a.HalfWidth, a.HalfHeight, a.HalfLength );
			//float aX = a.HalfWidth;
            //float aY = a.HalfHeight;
            //float aZ = a.HalfLength;

			Vector3 bV = new Vector3( b.HalfWidth, b.HalfHeight, b.HalfLength );
            //float bX = b.HalfWidth;
            //float bY = b.HalfHeight;
            //float bZ = b.HalfLength;
            ...
            //Test the axes defines by entity A's rotation matrix.
            //A.X
            float rarb = aV.X + Vector3.Dot( ref bV, ref absBR.M1 );
            if (t.X > rarb)
            {
                distance = t.X - rarb;
                axis = new Vector3(-aO.M11, -aO.M12, -aO.M13);
            ...
            bR.M2.X = aO.M2.Dot( ref bO.M1 );
            bR.M2.Y = aO.M2.Dot( ref bO.M2 );
            bR.M2.Z = aO.M2.Dot( ref bO.M3 );
            absBR.M2.X = Math.Abs(bR.M2.X) + Toolbox.Epsilon;
            absBR.M2.Y = Math.Abs(bR.M2.Y) + Toolbox.Epsilon;
            absBR.M2.Z = Math.Abs(bR.M2.Z) + Toolbox.Epsilon;
            float tY = t.Y;
            ...
            //A.Y
            rarb = aV.Y + Vector3.Dot( ref bV, ref absBR.M2 );
            if (t.Y > rarb)
            {
                distance = t.Y - rarb;
                axis = new Vector3(-aO.M21, -aO.M22, -aO.M23);
            ...
            bR.M3.X = aO.M3.Dot( ref bO.M1 );
            bR.M3.Y = aO.M3.Dot( ref bO.M2 );
            bR.M3.Z = aO.M3.Dot( ref bO.M3 );
            absBR.M3.X = Math.Abs(bR.M31) + Toolbox.Epsilon;
            absBR.M3.Y = Math.Abs(bR.M32) + Toolbox.Epsilon;
            absBR.M3.Z = Math.Abs(bR.M33) + Toolbox.Epsilon;
            t.Z = tX * aO.M31 + tY * aO.M32 + t.Z * aO.M33;
            ...
            //Test the axes defines by entity B's rotation matrix.
            //B.X
            rarb = bV.X + aV.X * absBR.M1.X + aV.Y * absBR.M2.X + aV.Z * absBR.M3.X;
            float tl = t.X * bR.M11 + t.Y * bR.M21 + t.Z * bR.M31;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //B.Y
            rarb = bV.Y + aV.X * absBR.M1.Y + aV.Y * absBR.M2.Y + aV.Z * absBR.M3.Y;
            tl = t.X * bR.M12 + t.Y * bR.M22 + t.Z * bR.M32;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //B.Z
            rarb = bV.Z + aV.X * absBR.M13 + aV.Y * absBR.M23 + aV.Z * absBR.M33;
            tl = t.X * bR.M13 + t.Y * bR.M23 + t.Z * bR.M33;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //Now for the edge-edge cases.
            //A.X x B.X
            rarb = aV.Y * absBR.M31 + aV.Z * absBR.M21 +
				   bV.Y * absBR.M13 + bV.Z * absBR.M12;
            tl = t.Z * bR.M21 - t.Y * bR.M31;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.X x B.Y
            rarb = aV.Y * absBR.M32 + aV.Z * absBR.M22 +
				   bV.X * absBR.M13 + bV.Z * absBR.M11;
            tl = t.Z * bR.M22 - t.Y * bR.M32;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.X x B.Z
            rarb = aV.Y * absBR.M33 + aV.Z * absBR.M23 +
				   bV.X * absBR.M12 + bV.Y * absBR.M11;
            tl = t.Z * bR.M23 - t.Y * bR.M33;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.Y x B.X
            rarb = aV.X * absBR.M31 + aV.Z * absBR.M11 +
				   bV.Y * absBR.M23 + bV.Z * absBR.M22;
            tl = t.X * bR.M31 - t.Z * bR.M11;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.Y x B.Y
            rarb = aV.X * absBR.M32 + aV.Z * absBR.M12 +
				   bV.X * absBR.M23 + bV.Z * absBR.M21;
            tl = t.X * bR.M32 - t.Z * bR.M12;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.Y x B.Z
            rarb = aV.X * absBR.M33 + aV.Z * absBR.M13 +
				   bV.X * absBR.M22 + bV.Y * absBR.M21;
            tl = t.X * bR.M33 - t.Z * bR.M13;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.Z x B.X
            rarb = aV.X * absBR.M21 + aV.Y * absBR.M11 +
				   bV.Y * absBR.M33 + bV.Z * absBR.M32;
            tl = t.Y * bR.M11 - t.X * bR.M21;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.Z x B.Y
            rarb = aV.X * absBR.M22 + aV.Y * absBR.M12 +
				   bV.X * absBR.M33 + bV.Z * absBR.M31;
            tl = t.Y * bR.M12 - t.X * bR.M22;
            if (tl > rarb)
            {
                distance = tl - rarb;
            ...
            //A.Z x B.Z
            rarb = aV.X * absBR.M23 + aV.Y * absBR.M13 +
				   bV.X * absBR.M32 + bV.Y * absBR.M31;
            tl = t.Y * bR.M13 - t.X * bR.M23;
            if (tl > rarb)
            {
                distance = tl - rarb;
                                                           ...
                                                           out Vector3 c1, out Vector3 c2)
        {
            ...
            {
                float c = Vector3.Dot(ref d1, ref r);
                ...
                {
                    float b = Vector3.Dot(ref d1, ref d2);
                    float denom = a * e - b * b;
        ...
        {
            boxFace = new BoxFace();
			float xDot = orientation.M1.Dot( ref mtd );
            float yDot = orientation.M2.Dot( ref mtd );
            float zDot = orientation.M3.Dot( ref mtd );
            ...
            Matrix3x3.ToMatrix4X4(ref orientation, ref position, out worldTransform);
