        ...
        public override void UpdateTimeOfImpact(Collidable requester, float dt)
        {
                ...
                )
            {
                ...
                if (minimumRadiusA * minimumRadiusA < velocitySquared)
                {
                    ...
                    RayHit rayHit;
					Vector3 tmp;
					velocity.Invert( out tmp );
					testRay = new Ray( ref collidableA.worldTransform.Position, ref tmp );
                    if (GJKToolbox.CCDSphereCast(ref testRay, minimumRadiusA, collidableB.Shape, ref collidableB.worldTransform, timeOfImpact, out rayHit))
                ...
                if (minimumRadiusB * minimumRadiusB < velocitySquared)
                {
                    ...
                    RayHit rayHit;
					testRay = new Ray( ref collidableB.worldTransform.Position, ref velocity );
					if( GJKToolbox.CCDSphereCast( ref testRay, minimumRadiusB, collidableA.Shape, ref collidableA.worldTransform, timeOfImpact, out rayHit))
                        timeOfImpact = rayHit.T;
                }
