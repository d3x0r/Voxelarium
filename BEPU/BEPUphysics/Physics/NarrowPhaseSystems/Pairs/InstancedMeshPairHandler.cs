            ...
            if (convex.IsActive && convex.entity.PositionUpdateMode == PositionUpdateMode.Continuous)
            {
                ...
                if (minimumRadius * minimumRadius < velocitySquared)
                {
                    ...
                    for (int i = 0; i < MeshManifold.overlappedTriangles.Count; i++)
                    {
                        ...
                        RayHit rayHit;
						Ray testRay = new Ray( ref Toolbox.ZeroVector, ref velocity );
                        if (GJKToolbox.CCDSphereCast(ref testRay, minimumRadius, triangle, ref Toolbox.RigidIdentity, timeOfImpact, out rayHit) &&
