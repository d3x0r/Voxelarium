...
namespace BEPUphysics.Entities
{
        ...
        IEquatable<Entity>
    {
		...
		internal Quaternion conjOrientation = Quaternion.Identity;
		public Matrix3x3 orientationMatrix = Matrix3x3.Identity;
        ...
        public Quaternion Orientation
        {
            ...
            set
            {
				...
				Quaternion.Conjugate( ref orientation, out conjOrientation );
        ...
        public Vector3? Gravity
        {
            ...
            set
            {
                if (value != null)
                {
                    if (!hasPersonalGravity || !value.Value.Equals( ref personalGravity ) )
                    {
        ...
        public void ApplyAngularImpulse(ref Vector3 impulse)
        {
            ...
            MathChecker.Validate(angularMomentum);
#else
            angularVelocity.X += impulse.X * inertiaTensorInverse.M1.X + impulse.Y * inertiaTensorInverse.M2.X + impulse.Z * inertiaTensorInverse.M3.X;
            angularVelocity.Y += impulse.X * inertiaTensorInverse.M1.Y + impulse.Y * inertiaTensorInverse.M2.Y + impulse.Z * inertiaTensorInverse.M3.Y;
            angularVelocity.Z += impulse.X * inertiaTensorInverse.M1.Z + impulse.Y * inertiaTensorInverse.M2.Z + impulse.Z * inertiaTensorInverse.M3.Z;
#endif
