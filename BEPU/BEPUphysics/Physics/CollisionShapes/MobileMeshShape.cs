...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class MobileMeshShape : EntityShape
    {
        ...
        public MobileMeshShape(Vector3[] vertices, int[] indices, AffineTransform localTransform, MobileMeshSolidity solidity)
        {
            ...
            data.worldTransform.Translation.Sub( ref shapeDistributionInformation.Center, out data.worldTransform.Translation );
        ...
        TriangleSidedness ComputeSolidSidednessHelper(Ray ray)
        {
            ...
            if (triangleMesh.Tree.GetOverlaps(ray, hitList))
            {
                ...
                if (hits.Count % 2 == 0)
                {
					...
					Vector3 normal; Vector3.CrossDiff2( ref vA, ref vB, ref vA, ref vC, out normal);
                    if (Vector3.Dot( ref normal, ref ray.Direction) < 0)
        ...
        private ShapeDistributionInformation ComputeVolumeDistribution(TransformableMeshData data)
        {
            ...
            for (int i = 0; i < data.indices.Length; i += 3)
            {
				...
				Vector3 tmp;
				vA.Add2( ref vB, ref vC, out tmp );
                shapeInformation.Center.AddScaled( ref tmp, perVertexWeight, out shapeInformation.Center );
            ...
            shapeInformation.Center.Mult( 1/totalWeight, out shapeInformation.Center );
