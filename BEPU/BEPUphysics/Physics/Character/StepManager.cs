...
namespace BEPUphysics.Character
{
    ...
    public class StepManager
    {
        ...
        public bool TryToStepDown(out Vector3 newPosition)
        {
            ...
            Plane plane = new Plane( ref normal, SupportFinder.SupportRayData.Value.HitData.Location.Dot( ref normal));
            ...
            try
            {
                ...
                if (Toolbox.GetRayPlaneIntersection(ref ray, ref plane, out hitT, out intersection))
                {
                    ...
                    characterBody.Position.AddScaled( ref down, currentOffset, out candidatePosition );
                                                ...
                                                out hintOffset))
                    {
                        case CharacterContactPositionState.Accepted:
                            ...
                            {
                                characterBody.Position.AddScaled( ref down, currentOffset, out newPosition );
                                return true;
                            }
                            else
                            {
            ...
            out float hintOffset)
        {
            ...
            if (tractionContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    ...
                    hintOffset = -Vector3.Dot( ref supportContact.Contact.Normal, ref down ) * supportContact.Contact.PenetrationDepth;
                ...
                else if (supportState == CharacterContactPositionState.TooDeep)
                {
                    ...
                    hintOffset = Math.Min(0, .001f - Vector3.Dot(ref supportContact.Contact.Normal, ref down ) * supportContact.Contact.PenetrationDepth);
        ...
        void FindUpStepCandidates(RawList<ContactData> outputStepCandidates, ref Vector3 down)
        {
            ...
            foreach (var c in SupportFinder.SideContacts)
            {
                ...
                if (dot > 0)
                {
					...
					Vector3 tmp;
					c.Contact.Position.Sub( ref characterBody.position, out tmp );
					dot = Vector3.Dot(ref down, ref tmp );
        ...
        bool TryToStepUsingContact(ref ContactData contact, ref Vector3 down, out Vector3 newPosition)
        {
            ...
            if (QueryManager.RayCastHitAnything(ref ray, length))
            {
            ...
            if (QueryManager.RayCastHitAnything(ref ray, upLength))
            {
                newPosition = new Vector3();
                return false;
            }
            ...
            //Intersect the ray against the plane defined by the support hit.
            Vector3 intersection;
            dot = Vector3.Dot(ref earliestHit.Location, ref supportNormal );
            Plane plane = new Plane( ref supportNormal, dot);
            Vector3 candidatePosition;
            ...
            try
            {
                ...
                if (Toolbox.GetRayPlaneIntersection(ref downRay, ref plane, out hitT, out intersection))
                {
                    ...
                    if (currentOffset > lowestBound)
                        lowestBound = currentOffset;
					Vector3 tmp;
					characterBody.position.AddScaled( ref down, currentOffset, out tmp );
					tmp.Add( ref horizontalOffset, out candidatePosition );
                                              ...
                                              out hintOffset))
                    {
                        case CharacterContactPositionState.Accepted:
                            ...
                            if (currentOffset < 0 && currentOffset > -maximumStepHeight - CollisionDetectionSettings.AllowedPenetration)
                            {
								...
								characterBody.Position.AddScaled( ref down, Math.Max( -maximumStepHeight, currentOffset ), out tmp );
                                tmp.Add( ref horizontalOffset, out newPosition );
                                return true;
                            }
                            else
                            {
        ...
        bool IsObstructiveToUpStepping(ref Vector3 sideNormal, ref ContactData contact)
        {
            ...
            foreach (var c in SupportFinder.SideContacts)
            {
                dot = c.Contact.Normal.Dot(ref contact.Normal);
                float depth = dot * c.Contact.PenetrationDepth;
