    ...
    public class CharacterController : Updateable, IBeforeSolverUpdateable
    {
        ...
        public Vector3 ViewDirection
        {
            ...
            set
            {
                ...
                {
					Vector3 tmp;
					Body.OrientationMatrix.M2.Invert( out tmp ); // turn Up into Down
                    Vector3.Cross(ref tmp, ref Toolbox.UpVector, out value);
                    ...
                    {
						Vector3.Cross(ref tmp, ref Toolbox.ForwardVector, out value );
            ...
            try
            {
                ...
                if (tryToJump)
                {
                    ...
                    if (SupportFinder.HasTraction)
                    {
						...
						Vector3 tmp;
						Body.OrientationMatrix.M2.Invert( out tmp );
                        float currentDownVelocity = Vector3.Dot( ref tmp, ref relativeVelocity );
                        //Target velocity is JumpSpeed.
                        float velocityChange = Math.Max(jumpSpeed + currentDownVelocity, 0);
                        ApplyJumpVelocity(ref supportData, ref tmp, -velocityChange, ref relativeVelocity);
        ...
        void CorrectContacts()
        {
            ...
            foreach (var pair in Body.CollisionInformation.Pairs)
            {
                foreach (var contactData in pair.Contacts)
                {
					...
					Vector3 offset; contact.Position.Sub( ref Body.position, out offset );
        ...
        void ComputeRelativeVelocity(ref SupportData supportData, out Vector3 relativeVelocity)
        {
            ...
            if (SupportFinder.HasSupport)
            {
                ...
                if (entityCollidable != null)
                {
                    ...
                    try
                    {
                        entityVelocity = Toolbox.GetVelocityOfPoint(ref supportData.Position, ref entityCollidable.Entity.position, ref entityCollidable.Entity.linearVelocity, ref entityCollidable.Entity.angularVelocity);
                    }
                    finally
                    {
                        if (locked)
        ...
        void ApplyJumpVelocity(ref SupportData supportData, ref Vector3 velocityChange, ref Vector3 relativeVelocity)
        {
			Vector3 tmp;
