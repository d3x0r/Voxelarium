    ...
    public class SupportFinder
    {
            ...
            else
            {
                ...
                if (SupportRayData != null)
                {
                    supportData = new SupportData
                    {
                        ...
                        Depth = SupportRayData.Value.HitData.Normal.Dot( ref down ) * (BottomDistance - SupportRayData.Value.HitData.T),
        ...
        private void UpdateVerticalSupportData(ref Vector3 down, ref Vector3 movementDirection)
        {
            if (HasTraction)
            {
                ...
                verticalSupportData.Depth = SupportRayData.Value.HitData.Normal.Dot( ref down ) * (BottomDistance - SupportRayData.Value.HitData.T);
        ...
        public void UpdateSupports(ref Vector3 movementDirection)
        {
            ...
            if (!HasSupport && hadTraction)
            {
                ...
                if (!HasTraction)
                {
                    ...
                    Ray ray = new Ray( ref bodyPosition, ref downDirection );
                ...
                {
					Vector3 tmp;
					characterBody.Position.AddScaled( ref movementDirection, rayCastInnerRadius, out tmp );
					Ray ray = new Ray(ref tmp, ref downDirection);
                    ...
                    ray.Position.Sub( ref obstructionRay.Position, out obstructionRay.Direction );
                    if (!QueryManager.RayCastHitAnything(ref obstructionRay, 1))
                    {
                ...
                if (!HasTraction && tryingToMove)
                {
					...
					Vector3 tmp;
					bodyPosition.Sub( ref horizontalOffset, out tmp );
                    Ray ray = new Ray(ref tmp, ref downDirection);
                ...
                if (!HasTraction && tryingToMove)
                {
                    ...
                    Vector3.Multiply(ref horizontalOffset, rayCastInnerRadius, out horizontalOffset);
					Vector3 tmp;
					bodyPosition.Add( ref horizontalOffset, out tmp );
                    Ray ray = new Ray( ref tmp, ref downDirection);
