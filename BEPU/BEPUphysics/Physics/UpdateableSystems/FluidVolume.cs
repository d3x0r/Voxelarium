    ...
    public class FluidVolume : Updateable, IDuringForcesUpdateable, ICollisionRulesOwner
    {
        ...
        public Vector3 FlowDirection
        {
            ...
            set
            {
                ...
                if (length > 0)
                {
                    value.Mult( 1 / length, out flowDirection );
                }
                else
        ...
        public void RecalculateBoundingBox()
        {
            ...
            foreach (var tri in SurfaceTriangles)
            {
                ...
                points.Add(tri[2]);
				Vector3 tmp;
                tri[0].AddScaled( ref upVector, - MaxDepth, out tmp );
				points.Add( tmp );
				tri[1].AddScaled( ref upVector, - MaxDepth, out tmp );
				points.Add( tmp );
				tri[2].AddScaled( ref upVector, - MaxDepth, out tmp );
				points.Add( tmp );
			}
        ...
        void AnalyzeEntry(int i)
        {
            ...
            if (entityCollidable != null && entityCollidable.IsActive && entityCollidable.entity.isDynamic && CollisionRules.collisionRuleCalculator(this, entityCollidable) <= CollisionRule.Normal)
            {
                ...
                if (submergedVolume > 0)
                {
                    ...
                    if (FlowForce != 0)
                    {
                        float dot = Math.Max(Vector3.Dot(ref entityCollidable.entity.linearVelocity, ref flowDirection), 0);
                        if (dot < MaxFlowSpeed)
                        {
                            FlowDirection.Mult( Math.Min(FlowForce, (MaxFlowSpeed - dot) * entityCollidable.entity.mass) * dt * fractionSubmerged, out force );
